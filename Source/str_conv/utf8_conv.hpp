...
constexpr std::pair<ForwardIt, char32_t>
utf8_to_codepoint(ForwardIt first, ForwardIt last) noexcept {
  if (first != last) {
    ...
    if (c >= 0xC0u && first != last) {
      ...
      if (c <= 0xDFu) { // 110xxxxx 10xxxxxx
        if (auto c2 = static_cast<unsigned char>(*it);
            c2 >= 0x80u && c2 <= 0xBFu)
          return {++it,
                  static_cast<char32_t>(((c & 0x1Fu) << 6) | (c2 & 0x3Fu))};
      } else if (c <= 0xEFu) { // 1110xxxx 10xxxxxx 10xxxxxx
        if (auto c2 = static_cast<unsigned char>(*it);
            c2 >= 0x80u && c2 <= 0xBFu && ++it != last)
          if (auto c3 = static_cast<unsigned char>(*it);
              c3 >= 0x80u && c3 <= 0xBFu)
            return {++it,
                    static_cast<char32_t>(((c & 0x0Fu) << 12) |
                                          ((c2 & 0x3Fu) << 6) | (c3 & 0x3Fu))};
      } else if (c <= 0xF7u) { // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
        if (auto c2 = static_cast<unsigned char>(*it);
            c2 >= 0x80u && c2 <= 0xBFu && ++it != last)
          if (auto c3 = static_cast<unsigned char>(*it);
              c3 >= 0x80u && c3 <= 0xBFu && ++it != last)
            if (auto c4 = static_cast<unsigned char>(*it);
                c4 >= 0x80u && c4 <= 0xBFu)
              return {++it, static_cast<char32_t>(
                                ((c & 0x07u) << 18) | ((c2 & 0x3Fu) << 12) |
                                ((c3 & 0x3Fu) << 6) | (c4 & 0x3Fu))};
      }
    }
  }
...
template <typename ForwardIt>
constexpr std::pair<ForwardIt, char32_t>
utf16_to_codepoint(ForwardIt first, ForwardIt last) noexcept {
  if (first != last) {
    ...
    if (c < 0xD800u || c > 0xDFFFu) // xxxxxxxx xxxxxxxx
      ...
      ForwardIt it = first;
      if (auto c2 = static_cast<char16_t>(*it);
          c2 >= 0xDC00u && c2 <= 0xDFFFu) // low surrogate
        return {++it, static_cast<char32_t>(
                          (((c & 0x3FFu) << 10) | (c2 & 0x3FFu)) + 0x10000u)};
    }
  }
  return {first, 0xFFFDu};
}
...
template <typename ForwardIt>
constexpr std::pair<ForwardIt, char32_t>
utf32_to_codepoint(ForwardIt first, ForwardIt last) noexcept {
  if (first != last) {
    char32_t c = static_cast<char32_t>(*first);
    ++first;
    if (((c & 0xFFFFu) < 0xFFFEu) && (c < 0xD800u || c > 0xDFFFu) &&
        c < 0x10FFFEu)
      return {first, c};
  }
  return {first, 0xFFFDu};
...
constexpr OutputIt codepoint_to_utf8(OutputIt it, char32_t c) noexcept {
  if (c <= 0x7Fu) // 0xxxxxxx
    ...
    *it++ = static_cast<char>((c & 0x3Fu) | 0x80u);
             ...
             (c & 0xFFFFu) < 0xFFFEu) { // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
...
constexpr OutputIt codepoint_to_utf16(OutputIt it, char32_t c) noexcept {
  if (c < 0xFFFEu) { // xxxxxxxx xxxxxxxx
    ...
    *it++ = static_cast<char16_t>(c & 0xFFFFu);
             ...
             (c & 0xFFFFu) < 0xFFFEu) { // 110110xx xxxxxxxx, 110111xx xxxxxxxx
    c -= 0x10000u;
...
constexpr std::string_view utf8_trim(std::string_view sv) noexcept {
  ...
  constexpr std::size_t FSM[][5] = {
      {0u, 0u, 1u, 2u, 4u}, {0u, 0u, 1u, 2u, 4u}, {0u, 3u, 1u, 2u, 4u},
      {0u, 0u, 1u, 2u, 4u}, {0u, 5u, 1u, 2u, 4u}, {0u, 6u, 1u, 2u, 4u},
      {0u, 0u, 1u, 2u, 4u}, {0u, 8u, 1u, 2u, 4u}, {0u, 9u, 1u, 2u, 4u},
      {0u, 0u, 1u, 2u, 4u},
  };
