...
// TODO: Define std::unordered_map<char, effect_t> for all effects, plus each
// expansion. Faster, but produces duplicate global consts.
effect_t GetEffectFromChar(char ch, int Chip, bool *bValid) // // //
{
  ...
  *bValid = true;
  ...
  for (effect_t eff = EF_NONE; eff != EF_COUNT;
       eff = static_cast<effect_t>(eff + 1)) {
    if (EFF_CHAR[eff] == ch) {
      return eff;
    }
  }
