...
int C2A03Square::getDutyMax() const { return static_cast<int>(MAX_DUTY); }
...
int CNoiseChan::getDutyMax() const { return MAX_DUTY; }
...
bool CDPCMChan::HandleEffect(effect_t EffNum, unsigned char EffParam) {
  ...
  case EF_RETRIGGER:
    ...
    queueSample() sets mRetriggerCtr to mRetriggerPeriod + 1, which gets
    decremented to mRetriggerPeriod during the same frame by RefreshChannel().
