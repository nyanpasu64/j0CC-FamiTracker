...
CBinaryFileWriter::CBinaryFileWriter(CFile *pFile)
    : m_pFile(pFile), m_iDataWritten(0) {}
...
unsigned int CBinaryFileWriter::GetWritten() const { return m_iDataWritten; }
...
CChunkRenderBinary::CChunkRenderBinary(CFile *pFile)
    : CBinaryFileWriter(pFile), m_iSampleAddress(0) {}

void CChunkRenderBinary::StoreChunks(const std::vector<CChunk *> &Chunks) {
  std::for_each(Chunks.begin(), Chunks.end(),
                [this](CChunk *pChunk) { StoreChunk(pChunk); });
}

void CChunkRenderBinary::StoreSamples(
    const std::vector<const CDSample *> &Samples) {
  std::for_each(Samples.begin(), Samples.end(),
                [this](const CDSample *pSample) { StoreSample(pSample); });
}
  ...
  for (int i = 0; i < pChunk->GetLength(); ++i) {
    if (pChunk->GetType() == CHUNK_PATTERN) {
      const std::vector<char> &vec =
          pChunk->GetStringData(CCompiler::PATTERN_CHUNK_INDEX);
...
CChunkRenderNSF::CChunkRenderNSF(CFile *pFile, unsigned int StartAddr)
    : CBinaryFileWriter(pFile), m_iStartAddr(StartAddr), m_iSampleAddr(0) {}
...
void CChunkRenderNSF::StoreChunksBankswitched(
    const std::vector<CChunk *> &Chunks) {
  // Store chunks into NSF banks with bankswitching
  std::for_each(Chunks.begin(), Chunks.end(),
                [this](CChunk *pChunk) { StoreChunkBankswitched(pChunk); });
}

void CChunkRenderNSF::StoreSamples(
    const std::vector<const CDSample *> &Samples) {
...
void CChunkRenderNSF::StoreSamplesBankswitched(
    const std::vector<const CDSample *> &Samples) {
  ...
  std::for_each(
      ...
      [this](const CDSample *pSample) { StoreSampleBankswitched(pSample); });
}
...
void CChunkRenderNSF::StoreSampleBankswitched(const CDSample *pDSample) {
  ...
  if (m_iSampleAddr + SampleSize >= (unsigned)CCompiler::DPCM_SWITCH_ADDRESS) {
    // Allocate new bank
    if (GetRemainingSize() !=
        0x1000) // Skip if already on beginning of new bank
      AllocateNewBank();
    m_iSampleAddr = CCompiler::PAGE_SAMPLES;
  }
...
void CChunkRenderNSF::StoreChunkBankswitched(const CChunk *pChunk) {
  ...
  case CHUNK_PATTERN:
    // Switchable data
    while ((GetBank() + 1) <= pChunk->GetBank() &&
           pChunk->GetBank() > CCompiler::PATTERN_SWITCH_BANK)
      AllocateNewBank();
  }
...
int CChunkRenderNSF::GetBank() const { return GetWritten() >> 12; }
...
CChunkRenderNES::CChunkRenderNES(CFile *pFile, unsigned int StartAddr)
    : CChunkRenderNSF(pFile, StartAddr) {}
