...
public:
  ...
  blargg_err_t set_sample_rate(long samples_per_sec,
  ...
  // stereo is true, increments 'dest' one extra time after writing each sample,
...
// Optimized inline sample reader for custom sample formats and mixing of
// Blip_Buffer samples
class Blip_Reader {
public:
  ...
  // removed using Blip_Buffer::remove_samples().
...
inline void Blip_Synth<quality, range>::offset_resampled(
    blip_resampled_time_t time, int delta, Blip_Buffer *blip_buf) const {
  // Fails if time is beyond end of Blip_Buffer, due to a bug in caller code or
  // the need for a longer buffer as set by set_sample_rate().
  assert((long)(time >> BLIP_BUFFER_ACCURACY) < blip_buf->buffer_size_);
  delta *= impl.delta_factor;
  int phase =
      (int)(time >> (BLIP_BUFFER_ACCURACY - BLIP_PHASE_BITS) & (blip_res - 1));
...
template <int quality, int range>
void Blip_Synth<quality, range>::offset(blip_time_t t, int delta,
