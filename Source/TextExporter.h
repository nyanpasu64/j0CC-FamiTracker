...
public:
  ...
  static const CString &ExportCellText(const stChanNote &stCell,
...
private:                                                              // // //
  bool ImportHex(CString &sToken, int &i, int line, int column,
                 CString &sResult);
  CString ExportString(const CString &s);
  bool ImportCellText(CFamiTrackerDoc *pDoc, Tokenizer &t, unsigned int track,
                      unsigned int pattern, unsigned int channel,
                      unsigned int row, CString &sResult);
