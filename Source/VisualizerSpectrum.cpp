...
void CVisualizerSpectrum::Create(int Width, int Height) {
  ...
  std::fill(m_pBlitBuffer.get(), m_pBlitBuffer.get() + Width * Height,
            BG_COLOR); // // //
}
...
void CVisualizerSpectrum::Draw() {
  ...
  float Step =
      0.2f * (float(FFT_POINTS) / float(m_iWidth)) * m_iBarSize; // // //
    ...
    for (int y = 0; y < m_iHeight; ++y) {
      ...
      for (int x = 0; x < m_iBarSize; ++x) { // // //
        if (m_iBarSize > 1 && x == m_iBarSize - 1)
          Color = DIM(Color, 50);
        m_pBlitBuffer[(m_iHeight - 1 - y) * m_iWidth + i * m_iBarSize + x +
                      OFFSET] = y < level ? Color : BG_COLOR;
      }
    }
