...
CSizeEditor::CSizeEditor(CWnd *pParent)
    : CWnd(), m_iValue(0), m_iButtonPressed(0), m_pParentWnd(pParent),
      m_bSizeCursor(false) {}
...
void CSizeEditor::OnPaint() {
  ...
  CFont Font;
  Font.CreateFont(-11, 0, 0, 0, FW_NORMAL, FALSE, FALSE, 0, DEFAULT_CHARSET,
  ...
  // The '+'-button
  buttonRect =
      CRect(rect.right - rect.bottom, 2, rect.right - 2, rect.bottom - 2);
  dc.FillSolidRect(buttonRect, BUTTON_FACE);
...
int CSizeEditor::GetValue() const { return m_iValue; }
