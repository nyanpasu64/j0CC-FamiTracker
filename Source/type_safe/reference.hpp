...
namespace type_safe {
template <typename T, bool XValue = false> class object_ref;

/// \exclude
namespace detail {
template <typename T, bool XValue> struct rebind_object_ref_impl {
...
using rebind_object_ref =
    typename rebind_object_ref_impl<typename std::remove_reference<T>::type,
                                    XValue>::type;
} // namespace detail

/// A reference to an object of some type `T`.
///
/// Unlike [std::reference_wrapper]() it does not try to model reference
/// semantics, instead it is basically a non-null pointer to a single object.
/// This allows rebinding on assignment.
/// Apart from the different access syntax it can be safely used instead of a
/// reference, and is safe for all kinds of containers.
///
...
template <typename T, bool XValue /* = false*/> class object_ref {
  ...
  static_assert(!std::is_reference<T>::value,
                "pass the type without reference");
  static_assert(!XValue || !std::is_const<T>::value,
                "must not be const if xvalue reference");
...
public:
  ...
  /// \group deref
  constexpr reference_type get() const noexcept { return **this; }
  ...
  /// \effects Invokes the function with the referred object followed by the
  /// arguments. \returns A [ts::object_ref]() to the result of the function, if
  /// `*this` is an xvalue reference, the result is as well. \requires The
  /// function must return an lvalue or another [ts::object_ref]() object.
  template <typename Func, typename... Args>
  auto map(Func &&f, Args &&... args) -> detail::rebind_object_ref<
      decltype(detail::map_invoke(std::forward<Func>(f),
                                  std::declval<object_ref &>().get(),
                                  std::forward<Args>(args)...)),
      XValue> {
    using result = decltype(detail::map_invoke(std::forward<Func>(f), get(),
                                               std::forward<Args>(args)...));
    return detail::rebind_object_ref<result, XValue>(detail::map_invoke(
        std::forward<Func>(f), get(), std::forward<Args>(args)...));
  }
};

template <typename T, bool XValue> class reference_optional_storage;

template <typename T> struct optional_storage_policy_for;
...
/// \effects Calls the `operator()` of `f` passing it `*ref` and the additional
/// arguments.
template <typename T, bool XValue, typename Func, typename... Args>
void with(const object_ref<T, XValue> &ref, Func &&f,
...
template <typename T> constexpr object_ref<T> ref(T &obj) noexcept {
...
template <typename T> using xvalue_ref = object_ref<T, true>;

/// Creates a [ts::xvalue_ref]().
/// \returns A [ts::xvalue_ref]() to the given object.
template <typename T> constexpr xvalue_ref<T> xref(T &obj) noexcept {
...
///
/// It is a simple pointer + size pair that allows reference access to each
/// element in the array. An "array" here is any contiguous storage (so C
/// arrays, [std::vector](), etc.). It does not allow changing the size of the
/// array, only the individual elements. Like [ts::object_ref]() it can be
/// safely used in containers.
///
/// If the given type is `const`, it will only return a `const` reference to
/// each element, but then `XValue` must be `false`.
///
/// If `XValue` is `true`, dereferencing will [std::move()]() the object,
/// modelling a reference to an expiring lvalue.
/// \notes `T` is the type stored in the array, so `array_ref<int>` to reference
/// a contiguous storage of `int`s. \notes Unlike the other types it isn't
/// technically non-null, as it may contain an empty array. But the range
/// `[data(), data() + size)` will always be valid.
template <typename T, bool XValue = false> class array_ref {
  static_assert(!std::is_void<T>::value, "must not be void");
  static_assert(!std::is_reference<T>::value,
                "pass the type without reference");
  static_assert(!XValue || !std::is_const<T>::value,
...
public:
  ...
  /// \requires `begin <= end`.
  /// \group range
  array_ref(T *begin, T *end) noexcept : size_(0u) { assign(begin, end); }
  ...
  /// \group c_array
  template <std::size_t Size> void assign(T (&arr)[Size]) noexcept {
    begin_ = arr;
    size_ = Size;
  }
  ...
  /// \returns A (`rvalue` if `Xvalue` is `true`) reference to the `i`th element
  /// of the array. \requires `i < size()`.
  reference_type operator[](index_t i) const noexcept {
    DEBUG_ASSERT(static_cast<size_t &>(i) < size_,
...
/// \effects Calls the `operator()` of `f` passing it `ref[i]` and the
/// additional arguments for each valid index of the array.
template <typename T, bool XValue, typename Func, typename... Args>
void with(const array_ref<T, XValue> &ref, Func &f,
          Args &&... additional_args) {
  for (auto &&elem : ref)
...
/// \returns The reference created by forwarding the parameter(s) to the
/// constructor. \group array_ref_ref
...
/// \group array_ref_ref
template <typename T> array_ref<T> ref(T *begin, T *end) noexcept {
  return array_ref<T>(begin, end);
}

/// \group array_ref_ref
template <typename T> array_ref<T> ref(T *array, size_t size) noexcept {
  return array_ref<T>(array, size);
}

/// Creates a [ts::array_ref]() to `const`.
/// \returns The reference created by forwarding the parameter(s) to the
/// constructor. \group array_ref_cref
template <typename T, std::size_t Size>
...
template <typename T> using array_xvalue_ref = array_ref<T, true>;

/// Creates a [ts::array_xvalue_ref]().
/// \returns The reference created by forwarding the parameter(s) to the
/// constructor. \group array_xvalue_ref_ref
template <typename T, std::size_t Size>
...
template <typename T> array_xvalue_ref<T> xref(T *begin, T *end) noexcept {
  return array_xvalue_ref<T>(begin, end);
}

/// \group array_xvalue_ref_ref
template <typename T> array_xvalue_ref<T> xref(T *array, size_t size) noexcept {
...
struct compatible_return_type
    : std::integral_constant<
          bool, std::is_void<Required>::value ||
...
struct enable_matching_function {
  using type = typename std::enable_if<
      compatible_return_type<decltype(std::declval<Func &>()(
                                 std::declval<Args>()...)),
                             Return>::value,
      int>::type;
};
...
struct get_callable_tag {
  ...
  static matching_function_pointer_tag test(
      int, T &obj,
      typename enable_matching_function<decltype(+obj), Return, Args...>::type =
          0);

  template <typename T>
  static matching_functor_tag
  test(short, T &obj,
       typename enable_matching_function<T, Return, Args...>::type = 0);
...
using enable_function_tag = typename std::enable_if<
    std::is_same<typename get_callable_tag<Func, Return, Args...>::type,
                 Result>::value,
    int>::type;
} // namespace detail
...
public:
  ...
  function_ref(Return2 (*fptr)(Args2...),
               typename detail::enable_matching_function<decltype(fptr), Return,
                                                         Args...>::type = 0)
      : function_ref(detail::matching_function_pointer_tag{}, fptr) {}
  ...
  template <typename Functor,
            typename = detail::enable_function_tag<detail::matching_functor_tag,
                                                   Functor, Return, Args...>>
  ...
  /// For example, if `other` has signature `void(const char*)` it can refer to
  /// a function taking `std::string`. If this signature than accepts a type `T`
  /// implicitly convertible to `const char*`, calling this will call the
  /// function taking `std::string`, converting `T -> std::string`, even though
  /// such a conversion would be ill-formed otherwise. \param 1 \exclude
  template <typename Return2, typename... Args2>
