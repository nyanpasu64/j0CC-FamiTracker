...
namespace type_safe {
template <class StoragePolicy> class basic_optional;

/// \exclude
namespace detail {
template <class StoragePolicy> struct optional_storage {
  ...
  optional_storage(optional_storage &&other) noexcept(
      std::is_nothrow_move_constructible<
          typename StoragePolicy::value_type>::value) {
...
///
/// Some operations like [ts::basic_optional::map()]() change the type of an
/// optional. This traits controls which `StoragePolicy` is going to be used for
/// the new optional. You can for example requests a
/// [ts::compact_optional_storage]() for your type, simply specialize it and set
/// a `type` typedef. \module optional
template <typename T> struct optional_storage_policy_for { using type = void; };

/// Specialization of [ts::optional_storage_policy_for]() for
/// [ts::basic_optional]() itself.
///
/// It will simply forward to the same policy, so
/// `ts::optional_for<ts::optional<T>>` is simply `ts::optional<T>`, not
/// `ts::optional<ts::optional<T>>`. \module optional
template <class StoragePolicy>
struct optional_storage_policy_for<basic_optional<StoragePolicy>> {
  using type = StoragePolicy;
};

template <typename T, bool XValue = false> class reference_optional_storage;
...
/// \module optional
template <typename T> struct optional_storage_policy_for<T &> {
...
template <typename T> struct optional_storage_policy_for<T &&> {
  static_assert(sizeof(T) != sizeof(T),
                "no optional for rvalue references supported");
};
...
///
/// It is similar to [std::optional<T>]() but lacks some functions and provides
/// some others. It can be in one of two states: it contains a value of a
/// certain type or it does not (it is "empty").
///
...
/// * `StoragePolicy() noexcept` - a no-throw default constructor that
/// initializes it in the "empty" state
/// * `void create_value(Args&&... args)` - creates a value by forwarding the
/// arguments to its constructor
/// * `void create_value_explicit(T&& obj)` - creates a value requiring an
/// `explicit` constructor
/// * `void create_value(const StoragePolicy&/StoragePolicy&&)` - creates a
/// value by using the value stored in the other policy
/// * `void copy_value(const StoragePolicy&/StoragePolicy&&)` - similar to
/// above, but *this may contain a value already
/// * `void swap_value(StoragePolicy&)` - swaps the stored value (if any) with
/// the one in the other policy
/// * `void destroy_value() noexcept` - calls the destructor of the value,
/// afterwards the storage is "empty"
/// * `bool has_value() const noexcept` - returns whether or not there is a
/// value, i.e. `create_value()` has been called but `destroy_value()` has not
/// * `U get_value() (const)& noexcept` - returns a reference to the stored
/// value, U is one of the `XXX_reference` typedefs
/// * `U get_value() (const)&& noexcept` - returns a reference to the stored
/// value, U is one of the `XXX_reference` typedefs
/// * `U get_value_or(T&& val) [const&/&&]` - returns either `get_value()` or
/// `val` \module optional
template <class StoragePolicy>
...
public:
  ...
  ///
  /// It will use [ts::optional_storage_policy_for]() to determine whether a
  /// change of storage policy is needed. \notes If `U` is `void`, the result
  /// will be `void` as well. \notes Due to a specialization of
  /// [ts::optional_storage_policy_for](), if `U` is an optional itself, the
  /// result will be `U`, not an optional of an optional. \exclude target
  template <typename U>
  using rebind =
      detail::rebind_optional<U, typename StoragePolicy::template rebind<U>>;

private:
  storage &get_storage() TYPE_SAFE_LVALUE_REF noexcept {
    return static_cast<detail::optional_storage<StoragePolicy> &>(*this)
        .storage;
  }

  const storage &get_storage() const TYPE_SAFE_LVALUE_REF noexcept {
    return static_cast<const detail::optional_storage<StoragePolicy> &>(*this)
        .storage;
  }
  ...
  storage &&get_storage() && noexcept {
    return std::move(
        static_cast<detail::optional_storage<StoragePolicy> &>(*this).storage);
  }

  const storage &&get_storage() const &&noexcept {
    return std::move(
        static_cast<const detail::optional_storage<StoragePolicy> &>(*this)
            .storage);
  }
#endif
...
public:
  ...
  /// \requires The `create_value()` function of the `StoragePolicy` must accept
  /// `value`. \param 1 \exclude
  template <
      typename T,
      typename = typename std::enable_if<!std::is_same<
          typename std::decay<T>::type, basic_optional<storage>>::value>::type>
  basic_optional(T &&value, decltype(std::declval<storage>().create_value(
                                         std::forward<T>(value)),
                                     0) = 0) {
  ...
  /// \effects Creates it with a value by forwarding `value`.
  /// \throws Anything thrown by the constructor of `value_type`.
  /// \requires The `create_value_explicit()` function of the `StoragePolicy`
  /// must accept `value`. \param 1 \exclude
  template <typename T, typename std::enable_if<
                            !std::is_same<typename std::decay<T>::type,
                                          basic_optional<storage>>::value,
                            int>::type = 0>
  explicit basic_optional(
      T &&value, decltype(std::declval<storage>().create_value_explicit(
                              std::forward<T>(value)),
                          0) = 0) {
  ...
  /// Copy constructor.
  /// \effects If `other` does not have a value, it will be created without a
  /// value as well. If `other` has a value, it will be created with a value by
  /// copying `other.value()`. \throws Anything thrown by the copy constructor
  /// of `value_type` if `other` has a value. \notes This constructor will not
  /// participate in overload resolution, unless the `value_type` is copy
  /// constructible.
  ...
  /// \effects If `other` does not have a value, it will be created without a
  /// value as well. If `other` has a value, it will be created with a value by
  /// moving `other.value()`. \throws Anything thrown by the move constructor of
  /// `value_type` if `other` has a value. \notes `other` will still have a
  /// value after the move operation, it is just in a moved-from state./ \notes
  /// This constructor will not participate in overload resolution, unless the
  ...
  auto operator=(T &&value)
      -> decltype(std::declval<basic_optional<storage>>()
                      .get_storage()
                      .create_value(std::forward<T>(value)),
                  *this) {
  ...
  /// `emplace(other.value())` (this will always trigger the single parameter
  /// version). Otherwise will reset the optional to the empty state. \throws
  /// Anything thrown by the call to `emplace()`. \notes This operator will not
  /// participate in overload resolution, unless the `value_type` is copy
  /// constructible.
  ...
  /// \effects If `other` has a value, does something equivalent to
  /// `emplace(std::move(other).value())` (this will always trigger the single
  /// parameter version). Otherwise will reset the optional to the empty state.
  ...
  /// \effects Swap.
  /// If both `a` and `b` have values, swaps the values with their swap
  /// function. Otherwise, if only one of them have a value, moves that value to
  /// the other one and makes the moved-from empty. Otherwise, if both are
  /// empty, does nothing. \throws Anything thrown by the move construction or
  /// swap.
  ...
  //=== modifiers ===//
  /// \effects Destroys the value by calling its destructor, if there is any
  /// stored. Afterwards `has_value()` will return `false`. \output_section
  /// Modifiers
  void reset() noexcept {
    if (has_value())
  ...
  /// Then creates the value by perfectly forwarding `args...` to the
  /// constructor of `value_type`. \throws Anything thrown by the constructor of
  /// `value_type`. If this function is left by an exception, the optional will
  /// be empty. \notes If the `create_value()` function of the `StoragePolicy`
  /// does not accept the arguments, this function will not participate in
  /// overload resolution. \synopsis_return void
  template <typename... Args>
  auto emplace(Args &&... args) noexcept(
      std::is_nothrow_constructible<value_type, Args...>::value)
      -> decltype(std::declval<basic_optional<storage>>()
                      .get_storage()
                      .create_value(std::forward<Args>(args)...)) {
    reset();
    get_storage().create_value(std::forward<Args>(args)...);
  }

  /// \effects If `has_value()` is `false` creates it by calling the constructor
  /// with `arg` perfectly forwarded. Otherwise assigns a perfectly forwarded
  /// `arg` to `value()`. \throws Anything thrown by the constructor or
  /// assignment operator chosen. \notes This function does not participate in
  /// overload resolution unless there is an `operator=` that takes `arg`
  /// without an implicit user-defined conversion and the `create_value()`
  /// function of the `StoragePolicy` accepts the argument. \synopsis_return
  /// void
  template <typename Arg,
            typename = typename std::enable_if<detail::is_direct_assignable<
                decltype(std::declval<storage &>().get_value()),
                Arg &&>::value>::type>
  auto emplace(Arg &&arg) noexcept(
      std::is_nothrow_constructible<value_type, Arg>::value
          &&std::is_nothrow_assignable<value_type, Arg>::value)
      -> decltype(std::declval<basic_optional<storage>>()
                      .get_storage()
                      .create_value(std::forward<Arg>(arg))) {
    if (!has_value())
  ...
  bool has_value() const noexcept { return get_storage().has_value(); }
  ...
  /// \group value
  auto value() const &&
      noexcept -> decltype(std::declval<const storage &&>().get_value()) {
    DEBUG_ASSERT(has_value(), detail::precondition_error_handler{});
    return std::move(get_storage()).get_value();
  }
#endif

  /// \returns If it has a value, `value()`, otherwise `u` converted to the same
  /// type as `value()`. \requires `u` must be valid argument to the
  /// `value_or()` function of the `StoragePolicy`. \notes Depending on the
  /// `StoragePolicy`, this either returns a decayed type or a reference. \group
  /// value_or
  template <typename U>
  auto value_or(U &&u) const TYPE_SAFE_LVALUE_REF -> decltype(
      std::declval<const storage &>().get_value_or(std::forward<U>(u))) {
  ...
  /// calls the function with the value followed by the additional arguments
  /// perfectly forwarded. \returns A `basic_optional` rebound to the result
  /// type of the function, that is empty if `*this` is empty and contains the
  /// result of the function otherwise. \requires `f` must either be a function
  /// or function object of matching signature, or a member function pointer of
  /// the stored type with compatible signature. \notes Due to the way
  /// [ts::basic_optional::rebind]() works, if the result of the function is
  /// `void`, `map()` will return `void` as well, and if the result of the
  /// function is an optional itself, `map()` will return the optional
  /// unchanged. \unique_name *map \group map \exclude return
  template <typename Func, typename... Args>
  auto map(Func &&f, Args &&... args) TYPE_SAFE_LVALUE_REF
#if !TYPE_SAFE_USE_RETURN_TYPE_DEDUCTION
      -> rebind<decltype(detail::map_invoke(
          std::forward<Func>(f), this->value(), std::forward<Args>(args)...))>
#endif
  {
    using return_type = decltype(detail::map_invoke(
        std::forward<Func>(f), value(), std::forward<Args>(args)...));
    if (has_value())
      return rebind<return_type>(detail::map_invoke(
          std::forward<Func>(f), value(), std::forward<Args>(args)...));
    else
...
/// A optional compares never less to `nullopt`, `nullopt` compares less only if
/// the optional has a value. The other comparisons behave accordingly. \group
/// optional_comp_null Optional null comparison \module optional
...
/// \effects Calls the `operator()` of `f` passing it the value of `opt` and
/// additional arguments, if it has a value. Otherwise does nothing. \module
/// optional \param 3 \exclude
template <class Optional, typename Func, typename... Args,
          typename = typename std::enable_if<
              detail::is_optional<Optional>::value>::type>
...
//=== optional ===//
/// A `StoragePolicy` for [ts::basic_optional]() that is similar to
/// [std::optional<T>]()'s implementation.
///
/// It uses [std::aligned_storage]() and a `bool` flag whether a value was
/// created. \requires `T` must not be a reference. \module optional
/// \output_section Optional
template <typename T> class direct_optional_storage {
...
public:
  ...
  template <typename U> using rebind = direct_optional_storage<U>;

  /// \effects Initializes it in the state without value.
  direct_optional_storage() noexcept : empty_(true) {}

  /// \effects Calls the constructor of `value_type` by perfectly forwarding
  /// `args`. Afterwards `has_value()` will return `true`. \throws Anything
  /// thrown by the constructor of `value_type` in which case `has_value()` is
  /// still `false`. \requires `has_value() == false`. \notes This function does
  /// not participate in overload resolution unless `value_type` is
  /// constructible from `args`. \synopsis_return void
  template <typename... Args>
  auto create_value(Args &&... args) -> typename std::enable_if<
      std::is_constructible<value_type, Args &&...>::value>::type {
    ::new (as_void()) value_type(std::forward<Args>(args)...);
    empty_ = false;
  }

  /// \effects Creates a value by copy(1)/move(2) constructing from the value
  /// stored in `other`, if there is any. \group create_value
  ...
  /// \effects Copies the policy from `other`, by copy-constructing or assigning
  /// the stored value, if any. \throws Anything thrown by the copy constructor
  /// or copy assignment operator of `other`. \group copy_value \unique_name
  /// copy_value_assign \param Dummy \exclude \param 1 \exclude
  ...
  /// \exclude
  template <typename Dummy = T,
            typename std::enable_if<!std::is_copy_assignable<Dummy>::value,
                                    int>::type = 0>
  void copy_value(const direct_optional_storage &other) {
    if (has_value())
      destroy_value();
  ...
  /// \effects Copies the policy from `other`, by move-constructing or assigning
  /// the stored value, if any. \throws Anything thrown by the move constructor
  /// or move assignment operator of `other`. \unique_name
  /// copy_value_move_assign \group copy_value_move \param Dummy \exclude \param
  /// 1 \exclude
  template <typename Dummy = T,
            typename = typename std::enable_if<
                std::is_move_assignable<Dummy>::value>::type>
  ...
  /// \exclude
  template <typename Dummy = T,
            typename std::enable_if<!std::is_move_assignable<Dummy>::value,
                                    int>::type = 0>
  void copy_value(direct_optional_storage &&other) {
    if (has_value())
      destroy_value();
  ...
  bool has_value() const noexcept { return !empty_; }
  ...
  /// \requires `value_type` must be copy(1)/move(2) constructible and `u`
  /// convertible to `value_type`. \group get_value_or \param 1 \exclude
...
private:
  void *as_void() noexcept { return static_cast<void *>(&storage_); }
...
/// Uses [ts::optional_storage_policy_for]() to select the appropriate
/// [ts::basic_optional]().
///
...
/// \returns A new [ts::optional<T>]() with a value created by perfectly
/// forwarding `args` to the constructor. \module optional
...
struct hash<type_safe::basic_optional<StoragePolicy>> {
  ...
  std::size_t
  operator()(const type_safe::basic_optional<StoragePolicy> &opt) const
