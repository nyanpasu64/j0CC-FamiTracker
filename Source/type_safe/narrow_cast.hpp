...
/// \exclude
namespace detail {
template <typename T, class Policy> struct get_target_integer {
...
template <typename T> struct get_target_floating_point {
  using type = floating_point<T>;
};

template <typename T> struct get_target_floating_point<floating_point<T>> {
  using type = floating_point<T>;
};
...
TYPE_SAFE_FORCE_INLINE constexpr bool is_narrowing(
    const Source &source,
    typename std::enable_if<std::is_floating_point<Source>::value, int>::type =
        0)

{
  return sizeof(Target) <
             sizeof(Source) // no narrowing possible
...
/// \requires The value of `source` must be representable by the new target
/// type. \module types \param 2 \exclude
template <
    typename Target, typename Source,
    typename = typename std::enable_if<std::is_arithmetic<Source>::value>::type>
TYPE_SAFE_FORCE_INLINE constexpr Target
narrow_cast(const Source &source) noexcept {
...
/// \returns A [ts::integer]() with the same value as `source` but of a
/// different type. \requires The value of `source` must be representable by the
/// new target type. \notes `Target` can either be a specialization of the
/// `integer` template itself or a built-in integer type, the result will be
/// wrapped if needed. \module types \exclude return
template <typename Target, typename Source, class Policy>
TYPE_SAFE_FORCE_INLINE constexpr auto
narrow_cast(const integer<Source, Policy> &source) noexcept ->
    typename detail::get_target_integer<Target, Policy>::type {
  using target_integer =
      typename detail::get_target_integer<Target, Policy>::type;
  using target_t = typename target_integer::integer_type;
  return narrow_cast<target_t>(static_cast<Source>(source));
}

/// \returns A [ts::floating_point]() with the same value as `source` but of a
/// different type. \requires The value of `source` must be representable by the
/// new target type. \notes `Target` can either be a specialization of the
/// `floating_point` template itself or a built-in floating point type, the
/// result will be wrapped if needed. \module types \exclude return
template <typename Target, typename Source>
TYPE_SAFE_FORCE_INLINE constexpr auto
narrow_cast(const floating_point<Source> &source) noexcept ->
