...
//=== get_type_index ==//
template <typename T, typename... Ts> struct get_type_index_impl;
...
struct get_type_index_impl<T, T, Tail...>
...
using get_type_index =
    get_type_index_impl<T, typename std::decay<Types>::type...>;

template <class Union> struct destroy_union;
template <class Union> struct copy_union;
template <class Union> struct move_union;
} // namespace detail

/// Tag type so no explicit template instantiation of function parameters is
/// required. \module variant
template <typename T> struct union_type {
...
/// \notes Like the C `union` it does not automatically destroy the currently
/// stored type, and copy operations are deleted. \module variant
template <typename... Types> class tagged_union {
  ...
  template <class Union> friend struct detail::destroy_union;
  template <class Union> friend struct detail::copy_union;
  template <class Union> friend struct detail::move_union;
...
public:
  ...
  public:
    ...
    template <typename T> static constexpr bool is_valid() {
    ...
    /// `false` otherwise.
    explicit operator bool() const noexcept { return *this != type_id(); }
  ...
  //=== modifiers ===//
  /// \effects Creates a new object of given type by perfectly forwarding
  /// `args`. \throws Anything thrown by `T`s constructor, in which case the
  /// union will stay empty. \requires The union must currently be empty. and
  /// `T` must be a valid type and constructible from the arguments.
  template <typename T, typename... Args>
  void emplace(union_type<T>, Args &&... args) {
    constexpr auto index = type_id(union_type<T>{});
    // MSVC doesn't like operator!= of the strong typedef here, for some
    // reason...
    static_assert(get(index) != get(invalid_type),
                  "T must not be stored in variant");
  ...
  template <typename T> void destroy(union_type<T> type) noexcept {
    check(type);
    value(type).~T();
  ...
  const type_id &type() const noexcept { return cur_type_; }
  ...
  /// \group value
  template <typename T> T &&value(union_type<T> type) && noexcept {
    check(type);
    return std::move(*static_cast<T *>(get_memory()));
  }

  /// \group value
  template <typename T> const T &&value(union_type<T> type) const &&noexcept {
    check(type);
...
private:
  void *get_memory() noexcept { return static_cast<void *>(&storage_); }

  const void *get_memory() const noexcept {
    return static_cast<const void *>(&storage_);
  }

  template <typename T> void check(union_type<T> type) const noexcept {
...
constexpr typename tagged_union<Types...>::type_id
    tagged_union<Types...>::invalid_type;
...
private:
  template <typename T>
  static auto call(int, Union &&u, Func &&func, Args &&... args) -> decltype(
      std::forward<Func>(func)(std::forward<Union>(u).value(union_type<T>{}),
                               std::forward<Args>(args)...)) {
    return std::forward<Func>(func)(
        std::forward<Union>(u).value(union_type<T>{}),
        std::forward<Args>(args)...);
  }
...
/// the stored object. Else does nothing. \group union_with \module variant
...
void with(tagged_union<Types...> &u, Func &&f, Args &&... additional_args) {
  detail::with_union<Func &&, decltype(u),
                     typename tagged_union<Types...>::types,
...
template <class Union> struct destroy_union {
  struct visitor {
    template <typename T> void operator()(const T &, Union &u) {
...
template <class Union> struct copy_union {
  struct visitor {
    template <typename T> void operator()(const T &value, Union &dest) {
...
template <class Union> struct move_union {
  struct visitor {
    template <typename T> void operator()(T &&value, Union &dest) {
      dest.emplace(union_type<typename std::decay<T>::type>{},
                   std::move(value));
    }
  };
...
/// \module variant
template <typename... Types> void destroy(tagged_union<Types...> &u) noexcept {
  detail::destroy_union<tagged_union<Types...>>::destroy(u);
}

/// \effects Copies the type currently stored in one [ts::tagged_union]() to
/// another. This is equivalent to calling `dest.emplace(union_type<T>{},
/// org.value(union_type<T>{}))` (1)/ `dest.emplace(union_type<T>{},
/// std::move(org).value(union_type<T>{}))` (2), where `T` is the type currently
/// stored in the union. \throws Anything by the copy/move constructor in which
/// case nothing has changed. \requires `dest` must not store a type, and all
/// types must be copyable/moveable. \group union_copy_move \module variant
