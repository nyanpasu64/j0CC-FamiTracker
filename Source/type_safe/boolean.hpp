...
public:
  ...
  /// \notes This function does not participate in overload resolution if `T` is
  /// not a boolean type. \param 1 \exclude
...
/// `false` otherwise.
/// \notes (2)/(3) do not participate in overload resolution if `T` is not a
/// boolean type. \group boolean_comp_equal \module types
TYPE_SAFE_FORCE_INLINE constexpr bool operator==(const boolean &a,
...
/// \exclude
template <typename T, typename = detail::enable_boolean<T>>
TYPE_SAFE_FORCE_INLINE constexpr bool operator==(T a,
                                                 const boolean &b) noexcept {
  return static_cast<bool>(a) == static_cast<bool>(b);
}
...
/// `true` otherwise.
/// \notes (2)/(3) do not participate in overload resolution if `T` is not a
/// boolean type. \group boolean_comp_unequal \module types
TYPE_SAFE_FORCE_INLINE constexpr bool operator!=(const boolean &a,
                                                 const boolean &b) noexcept {
  return static_cast<bool>(a) != static_cast<bool>(b);
}
...
/// \effects Reads a `bool` from the [std::istream]() and assigns it to the
/// given [ts::boolean](). \output_section Input/output \module types
template <typename Char, class CharTraits>
std::basic_istream<Char, CharTraits> &
operator>>(std::basic_istream<Char, CharTraits> &in, boolean &b) {
  bool val;
  in >> val;
  b = val;
  return in;
...
/// \effects Converts the given [ts::boolean]() to `bool` and writes it to the
/// [std::ostream](). \module types
template <typename Char, class CharTraits>
std::basic_ostream<Char, CharTraits> &
operator<<(std::basic_ostream<Char, CharTraits> &out, const boolean &b) {
...
///
/// This allows using types where the comparison operator returns
/// [ts::boolean](), as it can not be implicitly converted to `bool` so, for
/// example, [std::less]() can not be used. \notes These comparison functors are
/// always transparent, i.e. can be used with two different types. \group
/// comparison_functors Comparison function objects \module types
...
/// \module types
template <> struct hash<type_safe::boolean> {
