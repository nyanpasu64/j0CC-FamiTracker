...
template <class VariantPolicy, typename Head, typename... Types>
struct is_variant_impl<basic_variant<VariantPolicy, Head, Types...>>
    : std::true_type {};

template <typename T>
using is_variant = is_variant_impl<typename std::decay<T>::type>;

template <typename... Types> struct traits_impl {
  using copy_constructible =
      all_of<std::is_copy_constructible<Types>::value...>;
  using move_constructible =
      all_of<std::is_move_constructible<Types>::value...>;
  ...
  using nothrow_move_assignable =
      all_of<nothrow_move_constructible::value,
             (std::is_move_assignable<Types>::value
                  ...
                  : true)...>;
  using nothrow_swappable = all_of<nothrow_move_constructible::value,
...
template <class VariantPolicy, class Union> struct copy_assign_union_value {
  struct visitor {
...
template <class VariantPolicy, class Union> struct move_assign_union_value {
  struct visitor {
    template <typename T, typename = typename std::enable_if<
...
//=== swap_union ===//
template <class VariantPolicy, class Union> struct swap_union {
  struct visitor {
    template <typename T> void operator()(T &, Union &a, Union &b) {
      constexpr auto id = typename Union::type_id(union_type<T>{});
  ...
  static void swap(Union &a, Union &b) { with(a, visitor{}, a, b); }
};

//=== map_union ===//
template <typename Functor, class Union> struct map_union {
  struct visitor {
    template <typename T, typename... Args>
    auto call(int, Union &res, Functor &&f, T &&value, Args &&... args)
        -> decltype((void)map_invoke(std::forward<Functor>(f),
                                     std::forward<T>(value),
    ...
    void call(short, Union &res, Functor &&, T &&value, Args &&...) {
      res.emplace(union_type<typename std::decay<T>::type>{},
                  std::forward<T>(value));
    }
...
//=== compare_variant ===//
template <class Variant> struct compare_variant {
...
//=== variant_storage ===//
template <class VariantPolicy, typename... Types> class variant_storage {
...
public:
  ...
  ~variant_storage() noexcept { destroy(storage_); }

  variant_storage &operator=(const variant_storage &other) {
    if (storage_.has_value() && other.storage_.has_value())
      copy_assign_union_value<VariantPolicy, tagged_union<Types...>>::assign(
  ...
  variant_storage &operator=(variant_storage &&other) noexcept(
    ...
    if (storage_.has_value() && other.storage_.has_value())
      move_assign_union_value<VariantPolicy, tagged_union<Types...>>::assign(
  ...
  tagged_union<Types...> &get_union() noexcept { return storage_; }

  const tagged_union<Types...> &get_union() const noexcept { return storage_; }
