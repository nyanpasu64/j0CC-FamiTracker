...
/// \exclude
namespace detail {
template <typename T> T &move_if(std::false_type, T &obj) noexcept {
  return obj;
}

template <typename T> T &&move_if(std::true_type, T &obj) noexcept {
  return std::move(obj);
}
} // namespace detail

/// A `StoragePolicy` for [ts::basic_optional]() that allows optional
/// references.
///
/// The actual `value_type` passed to the optional is [ts::object_ref](),
/// but the reference types are normal references, so `value()` will return a
/// `T&` and `value_or()` takes a fallback reference of the same type and
/// returns one of them. Assigning an optional will always change the target of
/// the reference. You cannot pass rvalues.
///
...
///
/// Depending on the const-ness of `T` is the reference to `const` or non-const
/// as well, unless `XValue` is true`, in which case `T` must not be `const`.
/// \module optional
...
public:
  ...
  template <typename U> using rebind = reference_optional_storage<U, XValue>;
  ...
  /// \notes This function only participates in overload resolution, if `U` is a
  /// reference compatible with `T`. \param 1 \exclude
  template <typename U,
            typename = decltype(std::declval<T *&>() = std::declval<U *>())>
  void create_value(
      const basic_optional<reference_optional_storage<U, XValue>> &ref) {
  ...
  void create_value(std::nullptr_t) noexcept { destroy_value(); }
  ...
  void destroy_value() noexcept { pointer_ = nullptr; }
  ...
  /// This must be given an lvalue of type `T` and it returns either an lvalue
  /// or an rvalue, depending on `XValue`. \exclude return
...
/// \module optional
template <typename T> optional_ref<T> opt_ref(optional<T> &opt) noexcept {
...
/// \module optional
template <typename T> optional_ref<T> opt_ref(T *ptr) noexcept {
  return ptr ? optional_ref<T>(*ptr) : nullopt;
}

/// \returns A [ts::optional_ref<T>]() to `const` to the pointee of `ptr` or
/// `nullopt`. \module optional
template <typename T> optional_ref<const T> opt_cref(const T *ptr) noexcept {
  return ptr ? optional_ref<const T>(*ptr) : nullopt;
}

/// A [ts::basic_optional]() that uses [ts::reference_optional_storage]() with
/// `XValue` being `true`. It is an optional reference to an xvalue, i.e. an
/// lvalue that can be moved from, like returned by `std::move()`. \notes `T` is
/// the type without the reference, i.e. `optional_xvalue_ref<int>`. \module
/// optional
template <typename T>
using optional_xvalue_ref = basic_optional<reference_optional_storage<T, true>>;

/// \returns A [ts::optional_xvalue_ref<T>]() to the pointee of `ptr` or
/// `nullopt`. \notes The pointee will be moved from when you call `value()`.
/// \module optional
template <typename T> optional_xvalue_ref<T> opt_xref(T *ptr) noexcept {
...
/// \returns A [ts::optional<T>]() containing a copy of the value of `ref`
/// created by move constructing if there is any value. \requires `T` must be
/// moveable. \module optional
template <typename T>
