...
namespace type_safe {
//=== constrained_type ===//
/// A `Verifier` for [ts::constrained_type]() that `DEBUG_ASSERT`s the
/// constraint.
///
/// If [TYPE_SAFE_ENABLE_PRECONDITION_CHECKS]() is `true`,
/// it will assert that the value fulfills the predicate and returns it
/// unchanged. If assertions are disabled, it will just return the value
/// unchanged. \output_section Constrained type
...
public:
  constrain_error()
      : std::logic_error(
            "Constraint of type_safe::constrained_type wasn't fulfilled") {}
};

/// A `Verifier` for [ts::constrained_type]() that throws an exception in case
/// of failure.
///
/// Unlike [ts::assertion_verifier](), it will *always* check the constrain.
/// If it is not fulfilled, it throws an exception of type
/// [ts::constrain_error](), otherwise return the original value unchanged.
...
auto verify_static_constrained(int) ->
    typename Constraint::template is_valid<T>;
...
/// The `Constraint` can also provide a nested template `is_valid<T>` to
/// statically check types. Those will be checked regardless of the `Verifier`.
///
...
/// \requires `T` must not be a reference, `Constraint` must be a moveable,
/// non-final class where no operation throws, and `Verifier` must provide a
/// `static` function `[const] T[&] verify(const T&, const Predicate&)`. The
/// return value is stored and it must always fulfill the predicate. It also
/// requires that no `const` operation on `T` may modify it in a way that the
/// predicate isn't fulfilled anymore. \notes Additional requirements of the
/// `Constraint` depend on the `Verifier` used. If not stated otherwise, a
/// `Verifier` in this library requires that the `Constraint` is a `Predicate`
/// for `T`.
...
public:
  ...
  explicit constexpr constrained_type(const value_type &value,
      ...
      : Constraint(std::move(predicate)),
        value_(Verifier::verify(value, get_constraint())) {}

  /// \group value_ctor
  explicit constexpr constrained_type(
      value_type &&value,
      constraint_predicate predicate =
          {}) noexcept(std::is_nothrow_constructible<value_type>::value
                           &&noexcept(Verifier::verify(std::move(value),
                                                       std::move(predicate))))
      : Constraint(std::move(predicate)),
        value_(Verifier::verify(std::move(value), get_constraint())) {}

  /// \exclude
  template <typename U, typename = typename std::enable_if<!detail::is_valid<
                            constraint_predicate, U>::value>::type>
  ...
  /// \effects Same as assigning `constrained_type(other,
  /// get_constraint()).release()` to the stored value. It will invoke
  /// copy(1)/move(2) constructor followed by move assignment operator. \throws
  /// Anything thrown by the copy(1)/move(2) constructor or move assignment
  /// operator of `value_type`, or the `Verifier` if the `value` is invalid. If
  /// the `value` is invalid, nothing will be changed. \requires `Constraint`
  ...
  /// \group assign_value
  TYPE_SAFE_CONSTEXPR14 constrained_type &
  operator=(value_type &&other) noexcept(
  ...
  /// \requires `Constraint` must be copyable.
  TYPE_SAFE_CONSTEXPR14 constrained_type &
  operator=(const constrained_type &other) {
  ...
  friend TYPE_SAFE_CONSTEXPR14 void
  swap(constrained_type &a, constrained_type &b) noexcept(
  ...
  /// \returns A proxy object to provide verified write-access to the stored
  /// value. \notes This function does not participate in overload resolution if
  /// `T` is `const`.
...
private:
  ...
  TYPE_SAFE_CONSTEXPR14 value_type &get_non_const() noexcept { return value_; }
...
/// The value must not be changed by other means, it is thus perfect for
/// function parameters. \unique_name constrained_type_ref
template <typename T, class Constraint, class Verifier>
...
public:
  ...
  explicit constexpr constrained_type(T &value,
                                      constraint_predicate predicate = {})
      : Constraint(std::move(predicate)),
        ref_(&Verifier::verify(value, get_constraint())) {}
  ...
  /// \returns A proxy object to provide verified write-access to the referred
  /// value. \notes This function does not participate in overload resolution if
  /// `T` is `const`.
  template <typename Dummy = T, typename = typename std::enable_if<
                                    !std::is_const<Dummy>::value>::type>
...
private:
  ...
  TYPE_SAFE_CONSTEXPR14 value_type &get_non_const() noexcept { return *ref_; }
...
/// Alias for
/// [ts::constrained_type<T&>](standardese://ts::constrained_type_ref/).
...
/// A proxy class to provide write access to the stored value of a
/// [ts::constrained_type]().
///
...
public:
  using value_type =
      typename constrained_type<T, Constraint, Verifier>::value_type;
...
private:
  constrained_modifier(
      constrained_type<T, Constraint, Verifier> &value) noexcept
      : value_(&value) {}
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
  ...
  constexpr auto operator Op(                                                  \
      const constrained_type<T, Constraint, Verifier> &lhs,                    \
...
/// \returns A [ts::constrained_type]() with the given `value`,  `Constraint`
/// and `Verifier`. \unique_name constrain_verifier
...
constexpr auto constrain(T &&value, Constraint c)
  ...
  return constrained_type<typename std::decay<T>::type, Constraint, Verifier>(
      std::forward<T>(value), std::move(c));
}

/// Creates a [ts::constrained_type]() with the default verifier,
/// [ts::assertion_verifier](). \returns A [ts::constrained_type]() with the
/// given `value` and `Constraint`. \requires As it uses a `DEBUG_ASSERT` to
/// check constrain, the value must be valid. \unique_name constrain
...
constexpr auto sanitize(T &&value, Constraint c)
    -> constrained_type<typename std::decay<T>::type, Constraint,
                        throwing_verifier> {
  return constrained_type<typename std::decay<T>::type, Constraint,
                          throwing_verifier>(std::forward<T>(value),
                                             std::move(c));
}

/// With operation for [ts::constrained_type]().
/// \effects Calls `f` with a non-`const` reference to the stored value of the
/// [ts::constrained_type](). It checks that `f` does not change the validity of
/// the object. \notes The same behavior can be accomplished by using the
...
/// The `Constraint` - which does not need to be a predicate anymore - is a
/// "tag" to differentiate a type in different states. For example, you could
/// have a "sanitized" value and a "non-sanitized" value that have different
/// types, so you cannot accidentally mix them. \notes It is only intended if
/// the `Constraint` cannot be formalized easily and/or is expensive. Otherwise
/// [ts::constrained_type]() is recommended as it does additional runtime checks
/// in debug mode.
...
constexpr auto tag(T &&value, Constraint c)
  ...
  return tagged_type<typename std::decay<T>::type, Constraint>(
      std::forward<T>(value), std::move(c));
}
...
struct non_null {
  template <typename T> struct is_valid : std::true_type {};

  template <typename T> constexpr bool operator()(const T &ptr) const noexcept {
    return ptr != nullptr;
  }
};

template <> struct non_null::is_valid<std::nullptr_t> : std::false_type {};
...
class non_empty {
  ...
  template <typename T> constexpr bool is_empty(short, const T &t) const {
    return !empty(t);
  }
...
/// A `Constraint` for the [ts::constrained_type]().
///
/// A value of a pointer-like type is valid if the expression `!value` is
/// `false`.
struct non_invalid {
  template <typename T>
