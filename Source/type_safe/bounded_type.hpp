...
template <bool Cond, typename T, typename Bound> struct select_bound;

template <typename T, typename Bound> struct select_bound<true, T, Bound> {
  using type = wrapper<T>;
};

template <typename T, typename Bound> struct select_bound<false, T, Bound> {
  static_assert(
      std::is_convertible<decltype(std::declval<const T &>() < Bound::value),
                          bool>::value,
...
using upper_bound_t = typename std::conditional<Inclusive, less_equal<T, Bound>,
                                                less<T, Bound>>::type;

// checks that the value is greater than the lower bound
template <bool Inclusive, typename T, typename Bound>
using lower_bound_t =
    typename std::conditional<Inclusive, greater_equal<T, Bound>,
...
/// `LowerInclusive`/`UpperInclusive` control whether the lower/upper bound
/// itself is valid too.
template <typename T, bool LowerInclusive, bool UpperInclusive,
          typename LowerBound = dynamic_bound,
...
class bounded : detail::lower_bound_t<LowerInclusive, T, LowerBound>,
  ...
  static constexpr bool lower_is_dynamic =
      detail::is_dynamic<LowerBound>::value;
  static constexpr bool upper_is_dynamic =
      detail::is_dynamic<UpperBound>::value;
...
public:
  ...
  /// \effects Does nothing, a static bound is not stored.
  /// It will use `LowerBound::value` as lower bound and `UpperBound::value` as
  /// upper bound. \notes This constructor does not participate in overload
  /// resolution, unless both bounds are static, i.e. not
  /// [ts::constraints::dynamic_bound](). \param Condition \exclude \param 1
  /// \exclude
  ...
  /// if it is dynamic, a copy created by perfectly forwarding will be stored
  /// and used as bound. \notes This constructor does not participate in
  /// overload resolution, unless the arguments are convertible to the bounds.
  /// \param 2
  /// \exclude
  /// \param 3
  /// \exclude
  ...
  explicit constexpr bounded(U1 &&lower, U2 &&upper,
                             decltype(lower_type(std::forward<U1>(lower)),
                                      0) = 0,
                             decltype(upper_type(std::forward<U2>(upper)),
                                      0) = 0)
      : lower_type(std::forward<U1>(lower)),
        upper_type(std::forward<U2>(upper)) {}

  /// Does the bounds check.
  template <typename U> constexpr bool operator()(const U &u) const {
    return lower()(u) && upper()(u);
  }

  /// \returns The value of the lower bound.
  constexpr const typename lower_type::value_type &get_lower_bound() const
      noexcept {
    return lower().get_bound();
  }

  /// \returns The value of the upper bound.
  constexpr const typename upper_type::value_type &get_upper_bound() const
      noexcept {
...
/// A `Constraint` for the [ts::constrained_type]().
///
/// A value is valid if it is between two given bounds but not the bounds
/// themselves.
...
template <class Verifier, bool LowerInclusive, bool UpperInclusive, typename T,
          typename UpperBound>
struct bounded_type_impl<Verifier, LowerInclusive, UpperInclusive, T, T,
                         UpperBound> {
...
template <class Verifier, bool LowerInclusive, bool UpperInclusive, typename T,
          typename LowerBound>
struct bounded_type_impl<Verifier, LowerInclusive, UpperInclusive, T,
                         LowerBound, T> {
...
template <class Verifier, bool LowerInclusive, bool UpperInclusive, typename T,
          typename U1, typename U2>
using make_bounded_type = typename bounded_type_impl<
    Verifier, LowerInclusive, UpperInclusive, typename std::decay<T>::type,
    typename std::decay<U1>::type, typename std::decay<U2>::type>::type;
} // namespace detail

/// An alias for [ts::constrained_type]() that uses [ts::constraints::bounded]()
/// as its `Constraint`. \notes This is some type where the values must be in a
/// certain interval.
template <typename T, bool LowerInclusive, bool UpperInclusive,
...
using bounded_type =
    constrained_type<T,
                     constraints::bounded<T, LowerInclusive, UpperInclusive,
...
template <typename T, T Value> struct integer_bound {
  static constexpr T value = Value;
};

template <typename T, T Value> constexpr T integer_bound<T, Value>::value;
...
/// You can use it for example like this `ts::make_bounded(50, 0_bound,
/// 100_bound)`, to bound an integer between `0` and `100`. \returns A type
/// representing the given value, the value has type `long long` (1)/`unsigned
/// long long`(2). \group bound_lit
template <char... Digits>
constexpr auto operator"" _bound()
    -> lit_detail::integer_bound<long long,
                                 detail::parse<long long, Digits...>()> {
  return {};
}

/// \group bound_lit
template <char... Digits>
constexpr auto operator"" _boundu() -> lit_detail::integer_bound<
    unsigned long long, detail::parse<unsigned long long, Digits...>()> {
  return {};
}
} // namespace literals

/// Creates a [ts::bounded_type]() to a specified
/// [ts::constraints::closed_interval](). \returns A [ts::bounded_type]() with
/// the given `value` and lower and upper bounds, where the bounds are valid
/// values as well. \requires As it uses [ts::assertion_verifier](), the value
/// must be valid. \notes If this function is passed in dynamic values of the
/// same type as `value`, it will create a dynamic bound. Otherwise it must be
/// passed static bounds.
...
constexpr auto make_bounded(T &&value, U1 &&lower, U2 &&upper)
  ...
  return result_type(std::forward<T>(value),
                     typename result_type::constraint_predicate(
                         std::forward<U1>(lower), std::forward<U2>(upper)));
}

/// Creates a [ts::bounded_type]() to a specified
/// [ts::constraints::closed_interval](). \returns A [ts::bounded_type]() with
/// the given `value` and lower and upper bounds, where the bounds are valid
/// values as well. \throws A [ts::constrain_error]() if the `value` isn't
/// valid, or anything else thrown by the constructor. \notes This is meant for
/// sanitizing user input, using a recoverable error handling strategy. \notes
/// If this function is passed in dynamic values of the same type as `value`, it
/// will create a dynamic bound. Otherwise it must be passed static bounds.
template <typename T, typename U1, typename U2>
...
/// Creates a [ts::bounded_type]() to a specified
/// [ts::constraints::open_interval](). \returns A [ts::bounded_type]() with the
/// given `value` and lower and upper bounds, where the bounds are not valid
/// values. \requires As it uses [ts::assertion_verifier](), the value must be
/// valid. \notes If this function is passed in dynamic values of the same type
/// as `value`, it will create a dynamic bound. Otherwise it must be passed
/// static bounds.
template <typename T, typename U1, typename U2>
...
/// Creates a [ts::bounded_type]() to a specified
/// [ts::constraints::open_interval](), using [ts::throwing_verifier]().
/// \returns A [ts::bounded_type]() with the given `value` and lower and upper
/// bounds, where the bounds are not valid values. \throws A
/// [ts::constrain_error]() if the `value` isn't valid, or anything else thrown
/// by the constructor. \notes This is meant for sanitizing user input, using a
/// recoverable error handling strategy. \notes If this function is passed in
/// dynamic values of the same type as `value`, it will create a dynamic bound.
...
/// Returns a copy of `val` so that it is in the given
/// [ts::constraints::closed_interval](). \effects If it is not in the interval,
/// returns the bound that is closer to the value. \output_section clamped_type
template <typename T, typename LowerBound, typename UpperBound, typename U>
constexpr auto
clamp(const constraints::closed_interval<T, LowerBound, UpperBound> &interval,
      U &&val) -> typename std::decay<U>::type {
  return val < interval.get_lower_bound()
             ? static_cast<typename std::decay<U>::type>(
...
/// A `Verifier` for [ts::constrained_type]() that clamps the value to make it
/// valid.
///
...
struct clamping_verifier {
  ...
  static constexpr auto verify(Value &&val,
                               const constraints::less_equal<T, Bound> &p) ->
  ...
  template <typename Value, typename T, typename Bound>
  static constexpr auto verify(Value &&val,
                               const constraints::greater_equal<T, Bound> &p) ->
      typename std::decay<Value>::type {
    return p(val)
               ? std::forward<Value>(val)
               : static_cast<typename std::decay<Value>::type>(p.get_bound());
  ...
  template <typename Value, typename T, typename LowerBound,
            typename UpperBound>
...
/// An alias for [ts::constrained_type]() that uses
/// [ts::constraints::closed_interval]() as its `Constraint` and
/// [ts::clamping_verifier]() as its `Verifier`. \notes This is some type where
/// the values are always clamped so that they are in a certain interval.
...
/// Creates a [ts::clamped_type]() from the specified
/// [ts::constraints::closed_interval](). \returns A [ts::clamped_type]() with
/// the given `value` and lower and upper bounds, where the bounds are valid
/// values. \notes If this function is passed in dynamic values of the same type
/// as `value`, it will create a dynamic bound. Otherwise it must be passed
/// static bounds.
