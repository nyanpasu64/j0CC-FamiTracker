...
public:
  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T do_addition(const T &a,
                                                        const T &b) noexcept {
    return a + b;
  }

  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T
  do_subtraction(const T &a, const T &b) noexcept {
    return a - b;
  }

  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T
  do_multiplication(const T &a, const T &b) noexcept {
    return a * b;
  }

  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T do_division(const T &a,
                                                        const T &b) noexcept {
    return a / b;
  }

  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T do_modulo(const T &a,
                                                      const T &b) noexcept {
    return a % b;
  }
};
...
template <typename T>
constexpr bool will_addition_error(unsigned_integer_tag, const T &a,
                                   const T &b) {
  return std::numeric_limits<T>::max() - b < a;
}

template <typename T>
constexpr bool will_subtraction_error(signed_integer_tag, const T &a,
                                      const T &b) {
  return b > T(0) ? a < std::numeric_limits<T>::min() + b
                  : a > std::numeric_limits<T>::max() + b;
}
template <typename T>
constexpr bool will_subtraction_error(unsigned_integer_tag, const T &a,
                                      const T &b) {
  return a < b;
}

template <typename T>
constexpr bool will_multiplication_error(signed_integer_tag, const T &a,
                                         const T &b) {
  return a > T(0)
             ? (b > T(0) ? a > std::numeric_limits<T>::max() / b : // a, b > 0
                    b < std::numeric_limits<T>::min() / a)
             : // a > 0, b <= 0
             (b > T(0) ? a < std::numeric_limits<T>::min() / b
                       : // a <= 0, b > 0
                  a != T(0) &&
                      b < std::numeric_limits<T>::max() / a); // a, b <= 0
}
template <typename T>
constexpr bool will_multiplication_error(unsigned_integer_tag, const T &a,
                                         const T &b) {
  return b != T(0) && a > std::numeric_limits<T>::max() / b;
}
...
template <typename T>
constexpr bool will_division_error(unsigned_integer_tag, const T &,
                                   const T &b) {
  return b == T(0);
}
...
public:
  ...
  TYPE_SAFE_FORCE_INLINE static constexpr T
  do_multiplication(const T &a, const T &b) noexcept {
    return detail::will_multiplication_error(detail::arithmetic_tag_for<T>{}, a,
                                             b)
               ? DEBUG_UNREACHABLE(detail::precondition_error_handler{},
...
public:
  ...
  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T do_subtraction(const T &a,
                                                           const T &b) {
    return detail::will_subtraction_error(detail::arithmetic_tag_for<T>{}, a, b)
  ...
  template <typename T>
  TYPE_SAFE_FORCE_INLINE static constexpr T do_multiplication(const T &a,
                                                              const T &b) {
    return detail::will_multiplication_error(detail::arithmetic_tag_for<T>{}, a,
                                             b)
...
/// and is either [ts::undefined_behavior_arithmetic]() or
