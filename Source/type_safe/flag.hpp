...
///     {
///         do_sth(c);
///         was_newl = false; // separate step, easy to forget (I did here
///         originally!)
///     }
/// }
/// ```
...
///      else if (was_newl.try_reset()) // resets flag and returns whether value
///      changed
...
public:
  ...
  constexpr flag(T initial_state) noexcept
      : state_(static_cast<bool>(initial_state)) {}
...
/// \notes (2)/(3) do not participate in overload resolution unless `T` is a
/// boolean type. \group flag_equal \module types
...
/// (2)/(3) the flag is in the given state.
/// \notes (2)/(3) do not participate in overload resolution unless `T` is a
/// boolean type. \group flag_unequal \module types
constexpr bool operator!=(flag lhs, flag rhs) noexcept { return !(lhs == rhs); }
