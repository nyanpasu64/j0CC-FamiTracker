...
template <typename A, typename B> struct common_type {
  using type =
      typename std::conditional<std::is_same<A, B>::value, A,
                                typename std::common_type<A, B>::type>::type;
};

template <typename A> struct common_type<A, void> { using type = A; };

template <typename A> struct common_type<void, A> { using type = A; };

template <> struct common_type<void, void> { using type = void; };
...
struct visit_optional<Save, Visitor, Optional> {
  ...
  static void call_visitor(short, Visitor &&, Args &&...) {
    static_assert(!Save,
                  "call to visitor does not cover all possible combinations");
  }
...
struct visit_optional<Save, Visitor, Optional, Rest...> {
  template <typename... Args>
  static auto call(Visitor &&visitor, Optional &&opt, Rest &&... rest,
                   Args &&... args) ->
      typename common_type<
    ...
    return opt.has_value()
               ? visit_optional<Save, Visitor, Rest...>::call(
...
template <typename T> struct visitor_allow_incomplete {
  ...
  template <typename U> static std::false_type test(short);

  static const bool value =
      decltype(test<typename std::decay<T>::type>(0))::value;
};

template <typename Visitor, class... Optionals>
auto visit_optional_impl(Visitor &&visitor, Optionals &&... optionals)
    -> decltype(detail::visit_optional<
                !visitor_allow_incomplete<Visitor>::value,
...
/// \effects Effectively calls `visitor((optionals.has_value() ?
/// optionals.value() : nullopt)...)`, i.e. the `operator()` of `visitor`
/// passing it `sizeof...(Optionals)` arguments, where the `i`th argument is the
/// `value()` of the `i`th optional or `nullopt`, if it has none. If the
/// particular combination of types is not overloaded, the program is
/// ill-formed, unless the `Visitor` provides a member named
/// `incomplete_visitor`, then `visit()` does not do anything instead of the
/// error. \returns The result of the chosen `operator()`, its the type is the
/// common type of all possible combinations. \module optional \exclude return
/// \param 2
/// \exclude
template <typename Visitor, class... Optionals,
          typename std::enable_if<
              detail::all_of<detail::is_optional<Optionals>::value...>::value,
              int>::type = 0>
auto visit(Visitor &&visitor, Optionals &&... optionals) -> decltype(
    detail::visit_optional_impl(std::forward<Visitor>(visitor),
                                std::forward<Optionals>(optionals)...)) {
...
class visit_variant_impl<AllowIncomplete, Visitor> {
  ...
  static void call_impl(short, Visitor &&, Args &&...) {
    static_assert(AllowIncomplete,
                  "visitor does not cover all possible combinations");
  }
...
public:
  ...
  static auto call(Visitor &&visitor, Args &&... args)
      -> decltype(call_impl(0, std::forward<Visitor>(visitor),
                            std::forward<Args>(args)...)) {
    return call_impl(0, std::forward<Visitor>(visitor),
                     std::forward<Args>(args)...);
  }
};
...
class visit_variant_impl<AllowIncomplete, Visitor, Variant> {
  ...
  static auto call_type(Visitor &&visitor, variant_types<>, Variant &&variant,
    ...
    return visit_variant_impl<AllowIncomplete, Visitor>::call(
  ...
  template <typename RecursiveDecltype, typename Head, typename... Tail,
            typename... Args>
  static auto call_type(Visitor &&visitor, variant_types<Head, Tail...>,
    ...
    if (variant.has_value(variant_type<Head>{}))
      ...
      return RecursiveDecltype::template call_type<RecursiveDecltype>(
          std::forward<Visitor>(visitor), variant_types<Tail...>{},
          std::forward<Variant>(variant), std::forward<Args>(args)...);
  }
...
public:
  ...
  static auto call(Visitor &&visitor, Variant &&variant, Args &&... args)
      -> decltype(call_type<visit_variant_impl>(
          ...
          std::forward<Variant>(variant), std::forward<Args>(args)...)) {
    return visit_variant_impl::call_type<visit_variant_impl>(
        std::forward<Visitor>(visitor),
        typename std::decay<Variant>::type::types{},
        std::forward<Variant>(variant), std::forward<Args>(args)...);
  }
...
class visit_variant_impl<AllowIncomplete, Visitor, Variant, Rest...> {
  ...
  static auto call_type(Visitor &&visitor, variant_types<>, Variant &&variant,
    ...
    DEBUG_ASSERT(
        std::decay<Variant>::type::allow_empty::value && !variant.has_value(),
        precondition_error_handler{}, "variant in invalid state for visitor");
    return visit_variant_impl<AllowIncomplete, Visitor, Rest...>::call(
        std::forward<Visitor>(visitor), std::forward<Rest>(rest)...,
  ...
  static auto call_type(Visitor &&visitor, variant_types<>, Variant &&variant,
    ...
    return visit_variant_impl<AllowIncomplete, Visitor, Rest...>::call(
        std::forward<Visitor>(visitor), std::forward<Rest>(rest)...,
        std::forward<Args>(args)..., get_dummy_type(variant));
  }
...
public:
  template <typename... Args>
  static auto call(Visitor &&visitor, Variant &&variant, Rest &&... rest,
                   Args &&... args)
      -> decltype(call_type<visit_variant_impl>(
          std::forward<Visitor>(visitor),
          typename std::decay<Variant>::type::types{},
...
auto visit_variant(Visitor &&visitor, Variants &&... variants) -> decltype(
  ...
  return visit_variant_impl<
      visitor_allow_incomplete<Visitor>::value, Visitor &&,
      Variants &&...>::call(std::forward<Visitor>(visitor),
...
/// i.e. it calls the `operator()` of the `visitor` where the `i`th argument is
/// the currently stored value in the `i`th variant, perfectly forwarded. If the
/// `i`th variant is empty and it allows the empty state, it passes `nullvar` as
/// parameter, otherwise the behavior is undefined. If the particular
/// combination of types is not overloaded, the program is ill-formed, unless
/// the `Visitor` provides a member named `incomplete_visitor`, then `visit()`
/// does not do anything instead of the error. \returns The result of the chosen
/// `operator()`, its the type is the common type of all possible combinations.
/// \exclude return
/// \module variant
template <class Visitor, class... Variants,
          typename = typename std::enable_if<detail::all_of<
              detail::is_variant<Variants>::value...>::value>::type>
