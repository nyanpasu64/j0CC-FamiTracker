...
/// An improved `union` storing at most one of the given types at a time (or
/// possibly none).
///
/// It is an improved version of [std::variant]().
/// A big problem with variant is implementing the operation that changes the
/// type. It has to destroy the old value and then create the new one. But how
/// to handle an exception when creating the new type? There are multiple ways
/// of handling this, so it is outsourced in a policy. The variant policy is a
/// class that must have the following members:
/// * `allow_empty` - either [std::true_type]() or [std::false_type]().
/// If it is "true", the variant can be put in the empty state explicitly.
/// * `void change_value(variant_type<T>, tagged_union<Types...>&, Args&&...
/// args)` - changes the value and type. It will be called when the variant
/// already contains an object of a different type. It must destroy the old type
/// and create a new one with the given type and arguments. \module variant
...
public:
  ...
  /// [ts::copy](standardese://ts::copy_union/) (1) or
  /// [ts::move](standardese://ts::move_union/) (2). \throws Anything thrown by
  /// the copy (1)/move (2) constructor. \notes This constructor only
  /// participates in overload resolution, if all types are copy (1)/move (2)
  ...
  /// \group copy_move_ctor
  basic_variant(basic_variant &&) TYPE_SAFE_NOEXCEPT_DEFAULT(
      traits::nothrow_move_constructible::value) = default;

  /// Initializes it containing a new object of the given type.
  /// \effects Creates it by calling `T`s constructor with the perfectly
  /// forwarded arguments. \throws Anything thrown by `T`s constructor. \notes
  /// This constructor does not participate in overload resolution, unless `T`
  ...
  /// \effects Same as the type + argument constructor called with the decayed
  /// type of the argument and the object perfectly forwarded. \throws Anything
  /// thrown by `T`s copy/move constructor. \notes This constructor does not
  /// participate in overload resolution, unless `T` is a valid type for the
  /// variant and copy/move constructible. \param 1 \exclude
  template <typename T,
            typename = detail::enable_variant_type<union_t, T, T &&>>
  basic_variant(T &&obj)
      : basic_variant(variant_type<typename std::decay<T>::type>{},
                      std::forward<T>(obj)) {}

  /// Initializes it from a [ts::tagged_union]().
  /// \effects Copies the currently stored type of the union
  /// into the variant by calling the copy (1)/move (2) constructor of the
  /// stored type. \throws Anything thrown by the selected copy (1)/move (2)
  /// constructor. \requires If the variant policy does not allow the empty
  /// state, the union must not be empty. \group ctor_union
  explicit basic_variant(const tagged_union<HeadT, TailT...> &u) {
    DEBUG_ASSERT(allow_empty::value || u.has_value(),
  ...
  /// If this variant contains the same type and there is a copy (1)/move (2)
  /// assignment operator available, assigns the object to this object. Else
  /// forwards to the variant policy's `change_value()` function. \throws
  /// Anything thrown by either the copy (1)/move (2) assignment operator or
  /// copy (1)/move (2) constructor. If the assignment operator throws, the
  /// variant will contain the partially assigned object. If the constructor
  /// throws, the state depends on the variant policy. \notes This function does
  /// not participate in overload resolution, unless all types are copy (1)/move
  /// (2) constructible. \group copy_move_assign
  basic_variant &operator=(const basic_variant &) = default;
  /// \group copy_move_assign
  basic_variant &operator=(basic_variant &&) TYPE_SAFE_NOEXCEPT_DEFAULT(
      traits::nothrow_move_assignable::value) = default;
  ...
  /// * Both variants contain the same type, `T`. Then it calls swap on the
  /// stored type.
  /// * Both variants contain a type, but different types.
  /// Then it swaps the variant by move constructing the objects from one type
  /// to the other, using the variant policy.
  /// * Only one variant contains an object. Then it moves the value to the
  /// empty variant, and destroys it in the non-empty variant.
  ///
  /// \effects In either case, it will only call the swap() function or the move
  /// constructor. \throws Anything thrown by the swap function, in which case
  /// both variants contain the partially swapped values, or the mvoe
  /// constructor, in which case the exact behavior depends on the variant
  /// policy.
  friend void
  swap(basic_variant &a,
  ...
  /// \notes This function does not participate in overload resolution,
  /// unless `T` is a valid type for the variant and assignable from the
  /// argument without creating an additional temporary. \param 2 \exclude
  /// \param 3
  /// \exclude
  template <typename T, typename Arg,
            typename = typename std::enable_if<
                detail::is_direct_assignable<T, Arg &&>::value>::type,
  ...
  /// \notes This function does not participate in overload resolution,
  /// unless `T` is a valid type for the variant and constructible from the
  /// arguments. \param 2 \exclude
  template <typename T, typename... Args,
            typename = detail::enable_variant_type<union_t, T, Args &&...>>
...
public:
  //=== observers ===//
  /// \returns The type id representing the type of the value currently stored
  /// in the variant. \notes If it does not have a value stored, returns
  /// [*invalid_type]().
  type_id type() const noexcept { return storage_.get_union().type(); }
  ...
  /// \group has_value
  bool has_value() const noexcept { return storage_.get_union().has_value(); }
  ...
  /// `false` otherwise.
  /// \notes `T` must not necessarily be a type that can be stored in the
  /// variant.
  template <typename T> bool has_value(variant_type<T> type) const noexcept {
  ...
  /// \returns A (`const`) lvalue (1, 2)/rvalue (3, 4) reference to the stored
  /// object of the given type. \requires The variant must currently store an
  /// object of the given type, i.e. `has_value(type)` must return `true`.
  /// \group value
  /// \param 1
  /// \exclude
  template <typename T, typename = typename std::enable_if<
                            type_id::template is_valid<T>()>::type>
  ...
  optional_ref<const nullvar_t> optional_value(variant_type<nullvar_t>) const
      noexcept {
    return has_value() ? nullptr : type_safe::opt_ref(&nullvar);
  }

  /// \returns A (`const`) [ts::optional_ref]() (1,
  /// 2)/[ts::optional_xvalue_ref]() to the stored value of given type. If it
  /// stores a different type, returns a null reference. \group optional_value
  template <typename T>
  optional_ref<T> optional_value(variant_type<T> type) TYPE_SAFE_LVALUE_REF
      noexcept {
    return has_value(type)
               ? type_safe::opt_ref(&storage_.get_union().value(type))
               : nullptr;
  }
  ...
  optional_value(variant_type<T> type) const TYPE_SAFE_LVALUE_REF noexcept {
    return has_value(type)
               ? type_safe::opt_ref(&storage_.get_union().value(type))
               : nullptr;
  }
  ...
  template <typename T>
    ...
    return has_value(type)
               ? type_safe::opt_xref(&storage_.get_union().value(type))
               : nullptr;
  }

  /// \group optional_value
  template <typename T>
  optional_xvalue_ref<const T>
  optional_value(variant_type<T> type) const &&noexcept {
    return has_value(type)
               ? type_safe::opt_xref(&storage_.get_union().value(type))
               : nullptr;
  }
  ...
  /// \throws Anything thrown by `T`s copy (1)/move (2) constructor or the
  /// converting constructor. \notes `T` must not necessarily be a type that can
  /// be stored in the variant./ \notes This function does not participate in
  /// overload resolution, unless `T` is copy (1)/move (2) constructible and the
  /// fallback convertible to `T`. \group value_or \param 2 \exclude
  ...
  T value_or(variant_type<T> type, U &&other,
    ...
    return has_value(type) ? value(type)
  ...
  /// `(current-value.*std::forward<Functor>(f))(std::forward<Args>(args)...)`.
  ...
  /// Otherwise, if the function was called, it contains the result of the
  /// function. Otherwise, it is a copy of the current variant. \throws Anything
  /// thrown by the function or copy/move constructor, in which case the variant
  /// will be left unchanged, unless the object was already moved into the
  /// function and modified there. \requires The result of the function - if it
  /// is called - can be stored in the variant. \notes (1) will use the copy
  /// constructor, (2) will use the move constructor. The function does not
  /// participate in overload resolution, if copy (1)/move (2) constructors are
  /// not available for all types. \group map \param 1 \exclude \param 2
  /// \exclude
  template <typename Functor, typename... Args, typename Dummy = void,
            typename = typename std::enable_if<
                traits::copy_constructible::value, Dummy>::type>
  basic_variant map(Functor &&f, Args &&... args) const TYPE_SAFE_LVALUE_REF {
    basic_variant result(force_empty{});
    if (!has_value())
      return result;
    detail::map_union<Functor &&, union_t>::map(
        result.storage_.get_union(), storage_.get_union(),
  ...
  template <typename Functor, typename... Args, typename Dummy = void,
            typename = typename std::enable_if<
                traits::move_constructible::value, Dummy>::type>
  basic_variant map(Functor &&f, Args &&... args) && {
    basic_variant result(force_empty{});
    if (!has_value())
      return result;
    detail::map_union<Functor &&, union_t>::map(
        result.storage_.get_union(), std::move(storage_.get_union()),
        std::forward<Functor>(f), std::forward<Args>(args)...);
    DEBUG_ASSERT(result.has_value(), detail::assert_handler{});
    return result;
  }
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op, Expr, Expr2)                              \
  template <class VariantPolicy, typename Head, typename... Types>             \
  bool operator Op(const basic_variant<VariantPolicy, Head, Types...> &lhs,    \
                   nullvar_t) {                                                \
    return (void)lhs, Expr;                                                    \
  }                                                                            \
  /** \group variant_comp_null */                                              \
  template <class VariantPolicy, typename Head, typename... Types>             \
  bool operator Op(nullvar_t,                                                  \
...
/// A variant compares never less to `nullvar`, `nullvar` compares less only if
/// the variant has a value. The other comparisons behave accordingly. \group
/// variant_comp_null \module variant
...
///
/// A variant compares equal to a value, if it contains an object of the same
/// type and the object compares equal. A variant compares less to a value, if -
/// when it has a different type - the type id compares less than the type id of
/// the value, or - when it has the same type - the object compares less to the
/// value. The other comparisons behave accordingly. \notes The value must not
/// necessarily have a type that can be stored in the variant. \group
/// variant_comp_t \module variant
...
bool operator==(const basic_variant<VariantPolicy, Head, Types...> &lhs,
                const T &rhs) {
  return lhs.has_value(variant_type<T>{}) &&
         lhs.value(variant_type<T>{}) == rhs;
}
...
bool operator<(const basic_variant<VariantPolicy, Head, Types...> &lhs,
               const T &rhs) {
  constexpr auto id =
      typename basic_variant<VariantPolicy, Head, Types...>::type_id(
          variant_type<T>{});
  if (lhs.type() != id)
    return lhs.type() < id;
...
///
/// They compare equal if both store the same type (or none) and the stored
/// object compares equal. A variant is less than another if they store
/// mismatched types and the type id of the first is less than the other, or if
/// they store the same type and the stored object compares less. The other
/// comparisons behave accordingly. \module variant \group variant_comp
template <class VariantPolicy, typename Head, typename... Types>
bool operator==(const basic_variant<VariantPolicy, Head, Types...> &lhs,
                const basic_variant<VariantPolicy, Head, Types...> &rhs) {
...
/// If the functor is callable for the `T`, calls its `operator()` passing it
/// the stored object. Else does nothing. \module variant \group variant_with
...
void with(basic_variant<VariantPolicy, Head, Types...> &variant, Func &&func,
  ...
  with(detail::storage_access::get(variant).get_union(),
       std::forward<Func>(func), std::forward<Args>(additional_args)...);
}

/// \group variant_with
template <class VariantPolicy, typename Head, typename... Types, typename Func,
          typename... Args>
void with(const basic_variant<VariantPolicy, Head, Types...> &variant,
          Func &&func, Args &&... additional_args) {
  with(detail::storage_access::get(variant).get_union(),
       std::forward<Func>(func), std::forward<Args>(additional_args)...);
}
...
void with(basic_variant<VariantPolicy, Head, Types...> &&variant, Func &&func,
          Args &&... additional_args) {
  with(std::move(detail::storage_access::get(variant).get_union()),
       std::forward<Func>(func), std::forward<Args>(additional_args)...);
}
...
/// \module variant
template <typename Fallback> class fallback_variant_policy {
...
private:
  template <typename T, typename... Types, typename... Args>
  static auto change_value_impl(union_type<T> type,
                                tagged_union<Types...> &storage,
                                Args &&... args) noexcept ->
...
/// A variant policy for [ts::basic_variant]() that creates a variant with
...
/// \exclude
namespace detail {
template <bool ForceNonEmpty> class non_empty_variant_policy {
public:
  using allow_empty = std::false_type;
...
private:
  ...
  template <typename T, typename... Types>
  static void change_value_impl(union_type<T> type,
                                tagged_union<Types...> &storage, T &&obj) {
    destroy(storage);
...
/// A variant policy for [ts::basic_variant]() that creates a variant which is
/// rarely empty.
///
/// When changing the type of the variant, it will use a the move constructor
/// with a temporary. If the move constructor throws, the variant will be left
/// in the empty state. Putting it into the empty state explicitly is not
/// allowed. \module variant
using rarely_empty_variant_policy = detail::non_empty_variant_policy<false>;

/// A variant policy for [ts::basic_variant]() that creates a variant which is
/// never empty.
///
/// Similar to [ts::rarely_empty_variant_policy]() but when the move constructor
/// throws, it calls [std::terminate()](). \module variant
using never_empty_variant_policy = detail::non_empty_variant_policy<true>;

/// \exclude
namespace detail {
template <typename... Types> struct select_variant_policy {
  using type = basic_variant<rarely_empty_variant_policy, Types...>;
};

template <typename... Types> struct select_variant_policy<nullvar_t, Types...> {
...
///
/// If the first type is [ts::nullvar_t]() it will use the
/// [ts::optional_variant_policy](), which explicitly allows the empty state.
