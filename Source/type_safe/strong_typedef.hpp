...
///
/// Unlike regular typedefs, this does create a new type and only allows
/// explicit conversion from the underlying one. The `Tag` is used to
/// differentiate between different strong typedefs to the same type. It is
/// designed to be used as a base class and does not provide any operations by
/// itself. Use the types in the `strong_typedef_op` namespace to generate
/// operations and/or your own member functions.
///
/// Example:
/// ```cpp
/// struct my_handle
...
using underlying_type = decltype(detail::underlying_type(
    std::declval<typename std::decay<StrongTypedef>::type>()));
...
template <class StrongTypedef>
constexpr const underlying_type<StrongTypedef> &
get_underlying(const StrongTypedef &type) {
  return get(type);
}

template <class StrongTypedef>
TYPE_SAFE_CONSTEXPR14 underlying_type<StrongTypedef> &
get_underlying(StrongTypedef &type) {
...
template <class StrongTypedef,
          typename = typename std::enable_if<
              is_strong_typedef<StrongTypedef>::value>::type>
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op, Name, Result)                             \
  /** \exclude */                                                              \
  template <class StrongTypedef>                                               \
  constexpr Result operator Op(const Name<StrongTypedef> &lhs,                 \
                               const Name<StrongTypedef> &rhs) {               \
    return Result(detail::get_underlying<StrongTypedef>(                       \
        static_cast<const StrongTypedef &>(lhs))                               \
                      Op detail::get_underlying<StrongTypedef>(                \
                          static_cast<const StrongTypedef &>(rhs)));           \
  }                                                                            \
  /** \exclude */                                                              \
  template <class StrongTypedef>                                               \
  constexpr Result operator Op(Name<StrongTypedef> &&lhs,                      \
                               const Name<StrongTypedef> &rhs) {               \
    return Result(detail::get_underlying<StrongTypedef>(                       \
        static_cast<StrongTypedef &&>(lhs))                                    \
                      Op detail::get_underlying<StrongTypedef>(                \
                          static_cast<const StrongTypedef &>(rhs)));           \
  }                                                                            \
  /** \exclude */                                                              \
...
#define TYPE_SAFE_DETAIL_MAKE_OP_STRONGTYPEDEF_OTHER(Op, Name, Result)         \
  ...
  constexpr Result operator Op(const Name<StrongTypedef, OtherArg> &lhs,       \
                               Other &&rhs) {                                  \
    return Result(get(static_cast<const StrongTypedef &>(                      \
        lhs)) Op detail::forward_or_underlying(detail::forward<Other>(rhs)));  \
  }                                                                            \
  /** \exclude */                                                              \
  template <class StrongTypedef, typename OtherArg, typename Other,            \
            typename = detail::enable_if_convertible_same<Other &&, OtherArg>> \
  constexpr Result operator Op(Name<StrongTypedef, OtherArg> &&lhs,            \
                               Other &&rhs) {                                  \
    return Result(get(static_cast<StrongTypedef &&>(                           \
        lhs)) Op detail::forward_or_underlying(detail::forward<Other>(rhs)));  \
  }
...
#define TYPE_SAFE_DETAIL_MAKE_OP_OTHER_STRONGTYPEDEF(Op, Name, Result)         \
  ...
  template <class StrongTypedef, typename OtherArg, typename Other,            \
            typename = detail::enable_if_convertible_same<Other &&, OtherArg>> \
  constexpr Result operator Op(Other &&lhs,                                    \
                               const Name<StrongTypedef, OtherArg> &rhs) {     \
  ...
  template <class StrongTypedef, typename OtherArg, typename Other,            \
            typename = detail::enable_if_convertible_same<Other &&, OtherArg>> \
  constexpr Result operator Op(Other &&lhs,                                    \
                               Name<StrongTypedef, OtherArg> &&rhs) {          \
    return Result(detail::forward_or_underlying(detail::forward<Other>(lhs))   \
...
#define TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND(Op, Name)                            \
  /** \exclude */                                                              \
  template <class StrongTypedef>                                               \
  TYPE_SAFE_CONSTEXPR14 StrongTypedef &operator Op(                            \
      Name<StrongTypedef> &lhs, const Name<StrongTypedef> &rhs) {              \
  ...
  template <class StrongTypedef>                                               \
  TYPE_SAFE_CONSTEXPR14 StrongTypedef &operator Op(                            \
      Name<StrongTypedef> &lhs, Name<StrongTypedef> &&rhs) {                   \
    detail::get_underlying<StrongTypedef>(                                     \
        static_cast<StrongTypedef &>(lhs)) Op                                  \
...
#define TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND_MIXED(Op, Name)                      \
  ...
  template <class StrongTypedef, typename OtherArg, typename Other,            \
            typename = detail::enable_if_convertible_same<Other &&, OtherArg>> \
  TYPE_SAFE_CONSTEXPR14 StrongTypedef &operator Op(                            \
      Name<StrongTypedef, OtherArg> &lhs, Other &&rhs) {                       \
...
#define TYPE_SAFE_DETAIL_MAKE_STRONG_TYPEDEF_OP(Name, Op)                      \
  ...
  TYPE_SAFE_DETAIL_MAKE_OP_STRONGTYPEDEF_OTHER(                                \
      Op, mixed_##Name##_noncommutative, StrongTypedef)                        \
...
struct array_subscript {
  ...
  const Result &operator[](const Index &i) const {
    using type = underlying_type<StrongTypedef>;
    return static_cast<const type &>(
        static_cast<const StrongTypedef &>(*this))[i];
  }
};

template <class StrongTypedef, class Category, typename T,
          typename Distance = std::ptrdiff_t>
struct iterator : dereference<StrongTypedef, T, T *, const T *>,
                  increment<StrongTypedef> {
...
struct random_access_iterator
  ...
  /// \exclude
  friend Distance operator-(const StrongTypedef &lhs,
...
template <class StrongTypedef> struct input_operator {
  ...
  friend std::basic_istream<Char, CharTraits> &
  operator>>(std::basic_istream<Char, CharTraits> &in, StrongTypedef &val) {
    using type = underlying_type<StrongTypedef>;
