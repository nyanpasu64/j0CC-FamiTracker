...
namespace type_safe {
template <typename FloatT> class floating_point;
...
template <typename From, typename To>
using enable_safe_floating_point_conversion = typename std::enable_if<
    is_safe_floating_point_conversion<From, To>::value>::type;

template <typename From, typename To>
using fallback_safe_floating_point_conversion = typename std::enable_if<
    !is_safe_floating_point_conversion<From, To>::value>::type;

template <typename A, typename B>
struct is_safe_floating_point_comparison
    : std::integral_constant<
          bool, is_safe_floating_point_conversion<A, B>::value ||
                    is_safe_floating_point_conversion<B, A>::value> {};

template <typename A, typename B>
using enable_safe_floating_point_comparison = typename std::enable_if<
    is_safe_floating_point_comparison<A, B>::value>::type;

template <typename A, typename B>
using fallback_safe_floating_point_comparison = typename std::enable_if<
    !is_safe_floating_point_comparison<A, B>::value>::type;
...
template <typename A, typename B>
using fallback_floating_point_result = typename std::enable_if<
    !is_safe_floating_point_operation<A, B>::value>::type;
} // namespace detail

/// A type safe floating point class.
///
/// It is a tiny, no overhead wrapper over a standard floating point type.
/// It behaves exactly like the built-in types except it does not allow
/// narrowing conversions.
///
/// \requires `FloatT` must be a floating point type.
/// \notes It intentionally does not provide equality or increment/decrement
/// operators. \module types
template <typename FloatT> class floating_point {
...
public:
  ...
  /// \exclude
  template <typename T,
            typename = detail::enable_safe_floating_point_conversion<
  ...
  TYPE_SAFE_FORCE_INLINE constexpr floating_point(
      const floating_point<T> &val) noexcept
      : value_(static_cast<T>(val)) {}

  /// \exclude
  template <typename T,
            typename = detail::fallback_safe_floating_point_conversion<
                T, floating_point_type>>
  ...
  template <typename T,
            typename = detail::enable_safe_floating_point_conversion<
                T, floating_point_type>>
  TYPE_SAFE_FORCE_INLINE floating_point &
  operator=(const floating_point<T> &val) noexcept {
    value_ = static_cast<T>(val);
    return *this;
  }
  ...
  /// \group conversion
  TYPE_SAFE_FORCE_INLINE explicit constexpr operator floating_point_type() const
      noexcept {
    return value_;
  }
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
  ...
  template <typename T,                                                        \
            typename = detail::enable_safe_floating_point_conversion<          \
                T, floating_point_type>>                                       \
  TYPE_SAFE_FORCE_INLINE floating_point &operator Op(                          \
  ...
  template <typename T,
            typename = detail::enable_safe_floating_point_conversion<
                T, floating_point_type>>
  TYPE_SAFE_FORCE_INLINE floating_point &
  operator+=(const floating_point<T> &other) noexcept {
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
  ...
  TYPE_SAFE_FORCE_INLINE constexpr bool operator Op(                           \
      const A &a, const floating_point<B> &b) {                                \
  ...
  TYPE_SAFE_FORCE_INLINE constexpr bool operator Op(                           \
      const floating_point<A> &a, const B &b) {                                \
...
/// \returns The result of the comparison of the stored floating point value in
/// the [ts::floating_point](). \notes These functions do not participate in
/// overload resolution unless `A` and `B` are both floating point types. \group
/// float_comp Comparison operators \module types \param 2 \exclude
template <typename A, typename B,
          typename = detail::enable_safe_floating_point_comparison<A, B>>
TYPE_SAFE_FORCE_INLINE constexpr bool
operator<(const floating_point<A> &a, const floating_point<B> &b) noexcept {
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
  ...
  TYPE_SAFE_FORCE_INLINE constexpr auto operator Op(                           \
      const A &a, const floating_point<B> &b) noexcept                         \
  ...
  template <typename A, typename B>                                            \
  TYPE_SAFE_FORCE_INLINE constexpr auto operator Op(                           \
      const floating_point<A> &a,                                              \
      const B &b) noexcept->detail::floating_point_result_t<A, B> {            \
    return a Op floating_point<B>(b);                                          \
  }                                                                            \
  /** \exclude */                                                              \
  template <typename A, typename B,                                            \
            typename = detail::fallback_floating_point_result<A, B>>           \
  constexpr int operator Op(floating_point<A>, floating_point<B>) noexcept =   \
      delete;                                                                  \
  /** \exclude */                                                              \
  template <typename A, typename B,                                            \
            typename = detail::fallback_floating_point_result<A, B>>           \
...
/// \returns The result of the binary operation of the stored floating point
/// value in the [ts::floating_point](). The type is a [ts::floating_point]() of
/// the bigger floating point type. \notes These functions do not participate in
/// overload resolution, unless `A` and `B` are both floating point types.
/// \module types
/// \group float_binary_op Binary operations
template <typename A, typename B>
TYPE_SAFE_FORCE_INLINE constexpr auto
operator+(const floating_point<A> &a, const floating_point<B> &b) noexcept
    -> detail::floating_point_result_t<A, B> {
  return static_cast<A>(a) + static_cast<B>(b);
}
...
/// \effects Converts the given [ts::floating_point]() to the underlying
/// floating point and writes it to the [std::ostream](). \module types
template <typename Char, class CharTraits, typename FloatT>
...
/// \module types
template <typename FloatT> struct hash<type_safe::floating_point<FloatT>> {
  std::size_t operator()(const type_safe::floating_point<FloatT> &f) const
      noexcept {
