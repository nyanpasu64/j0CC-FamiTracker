...
struct is_safe_integer_operation
    : std::integral_constant<
          bool, detail::is_integer<A>::value && detail::is_integer<B>::value &&
                    std::is_signed<A>::value == std::is_signed<B>::value> {};

template <typename A, typename B>
struct integer_result_type
    : std::enable_if<
          is_safe_integer_operation<A, B>::value,
...
/// It behaves exactly like the built-in types except that narrowing conversions
/// are not allowed. It also checks against `unsigned` under/overflow in debug
/// mode and marks it as undefined for the optimizer otherwise.
///
/// A conversion is considered safe if both integer types have the same
/// signedness and the size of the value being converted is less than or equal
/// to the destination size.
///
/// \requires `IntegerT` must be an integral type except `bool` and `char` (use
/// `signed char`/`unsigned char`). \notes It intentionally does not provide the
/// bitwise operations. \module types
template <typename IntegerT, class Policy /* = arithmetic_policy_default*/>
class integer {
  static_assert(detail::is_integer<IntegerT>::value,
                "must be a real integer type");
...
public:
  ...
  /// \exclude
  template <typename T,
            typename = detail::enable_safe_integer_conversion<T, integer_type>>
  ...
  /// \exclude
  template <typename T, typename = detail::fallback_safe_integer_conversion<
                            T, integer_type>>
  ...
  TYPE_SAFE_FORCE_INLINE constexpr integer operator+() const { return *this; }
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
  ...
  template <typename T,                                                        \
            typename =                                                         \
                detail::enable_safe_integer_conversion<T, integer_type>>       \
  ...
  template <typename T,                                                        \
            typename =                                                         \
                detail::fallback_safe_integer_conversion<T, integer_type>>     \
...
template <typename T, class Policy> struct make_signed<integer<T, Policy>> {
...
template <typename T, class Policy> struct make_unsigned<integer<T, Policy>> {
...
template <typename Integer, typename = typename std::enable_if<
                                detail::is_integer<Integer>::value>::type>
TYPE_SAFE_FORCE_INLINE constexpr make_signed_t<Integer>
make_signed(const Integer &i) {
...
TYPE_SAFE_FORCE_INLINE constexpr make_signed_t<integer<Integer, Policy>>
make_signed(const integer<Integer, Policy> &i) {
...
template <typename Integer, typename = typename std::enable_if<
                                detail::is_integer<Integer>::value>::type>
TYPE_SAFE_FORCE_INLINE constexpr make_unsigned_t<Integer>
make_unsigned(const Integer &i) {
...
/// \module types
template <typename Integer, class Policy>
TYPE_SAFE_FORCE_INLINE constexpr make_unsigned_t<integer<Integer, Policy>>
make_unsigned(const integer<Integer, Policy> &i) {
...
template <typename SignedInteger,
          typename = typename std::enable_if<
              std::is_signed<SignedInteger>::value>::type>
TYPE_SAFE_FORCE_INLINE constexpr make_unsigned_t<SignedInteger>
abs(const SignedInteger &i) {
...
template <typename SignedInteger, class Policy,
          typename = typename std::enable_if<
              std::is_signed<SignedInteger>::value>::type>
TYPE_SAFE_FORCE_INLINE constexpr make_unsigned_t<integer<SignedInteger, Policy>>
abs(const integer<SignedInteger, Policy> &i) {
  return make_unsigned(i > 0 ? i : -i);
}
...
template <typename UnsignedInteger, class Policy,
          typename = typename std::enable_if<
              std::is_unsigned<UnsignedInteger>::value>::type>
TYPE_SAFE_FORCE_INLINE constexpr integer<UnsignedInteger, Policy>
abs(const integer<UnsignedInteger, Policy> &i) {
  return i;
}
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
  ...
  TYPE_SAFE_FORCE_INLINE constexpr bool operator Op(                           \
      const A &a, const integer<B, Policy> &b) {                               \
  ...
  template <typename A, class Policy, typename B,                              \
            typename = detail::enable_safe_integer_comparison<A, B>>           \
  TYPE_SAFE_FORCE_INLINE constexpr bool operator Op(                           \
      const integer<A, Policy> &a, const B &b) {                               \
...
/// \returns The result of the comparison of the stored integer value in the
/// [ts::integer](). \notes These functions do not participate in overload
/// resolution unless `A` and `B` are both integer types. \group int_comp
/// Comparison operators \module types \param 3 \exclude
template <typename A, typename B, class Policy,
          typename = detail::enable_safe_integer_comparison<A, B>>
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op)                                           \
   ...
   * \group int_binary_op */                                                   \
  template <typename A, typename B, class Policy>                              \
  TYPE_SAFE_FORCE_INLINE constexpr auto operator Op(                           \
      const A &a, const integer<B, Policy> &b)                                 \
...
/// \returns The result of the binary operation of the stored integer value in
/// the [ts::integer](). The type is a [ts::integer]() of the bigger integer
/// type. \notes These functions do not participate in overload resolution,
...
TYPE_SAFE_FORCE_INLINE constexpr auto operator+(const integer<A, Policy> &a,
  ...
  return Policy::template do_addition<type>(static_cast<A>(a),
                                            static_cast<B>(b));
}
TYPE_SAFE_DETAIL_MAKE_OP(+)
...
TYPE_SAFE_FORCE_INLINE constexpr auto operator-(const integer<A, Policy> &a,
    ...
    -> integer<detail::integer_result_t<A, B>, Policy> {
  using type = detail::integer_result_t<A, B>;
  return Policy::template do_subtraction<type>(static_cast<A>(a),
                                               static_cast<B>(b));
}
TYPE_SAFE_DETAIL_MAKE_OP(-)
...
TYPE_SAFE_FORCE_INLINE constexpr auto operator*(const integer<A, Policy> &a,
    ...
    -> integer<detail::integer_result_t<A, B>, Policy> {
  using type = detail::integer_result_t<A, B>;
  return Policy::template do_multiplication<type>(static_cast<A>(a),
                                                  static_cast<B>(b));
}
TYPE_SAFE_DETAIL_MAKE_OP(*)
...
TYPE_SAFE_FORCE_INLINE constexpr auto operator/(const integer<A, Policy> &a,
    ...
    -> integer<detail::integer_result_t<A, B>, Policy> {
  using type = detail::integer_result_t<A, B>;
  return Policy::template do_division<type>(static_cast<A>(a),
                                            static_cast<B>(b));
}
TYPE_SAFE_DETAIL_MAKE_OP(/)
...
//=== input/output ===/
/// \effects Reads an integer from the [std::istream]() and assigns it to the
/// given [ts::integer](). \module types \output_section Input/output
template <typename Char, class CharTraits, typename IntegerT, class Policy>
std::basic_istream<Char, CharTraits> &
operator>>(std::basic_istream<Char, CharTraits> &in,
...
/// \effects Converts the given [ts::integer]() to the underlying integer type
/// and writes it to th [std::ostream](). \module types
template <typename Char, class CharTraits, typename IntegerT, class Policy>
std::basic_ostream<Char, CharTraits> &
operator<<(std::basic_ostream<Char, CharTraits> &out,
...
struct hash<type_safe::integer<IntegerT, Policy>> {
  std::size_t operator()(const type_safe::integer<IntegerT, Policy> &i) const
      noexcept {
