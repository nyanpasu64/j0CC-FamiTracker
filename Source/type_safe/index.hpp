...
struct difference_t : strong_typedef<difference_t, ptrdiff_t>,
  ...
  /// \exclude
  template <
      typename T, class Policy,
      typename = typename std::enable_if<
          detail::is_safe_integer_conversion<T, std::ptrdiff_t>::value>::type>
  constexpr difference_t(integer<T, Policy> i) noexcept
      : strong_typedef(static_cast<T>(i)) {}
};
...
constexpr index_t operator+(const index_t &lhs,
                            const difference_t &rhs) noexcept {
  return index_t(make_unsigned(make_signed(get(lhs)) + get(rhs)));
}

/// \group index_distance_plus
constexpr index_t operator+(const difference_t &lhs,
                            const index_t &rhs) noexcept {
  return rhs + lhs;
}

/// \returns The given [ts::index_t]() advanced backwards by the given
/// [ts::distance_t](). \module types
constexpr index_t operator-(const index_t &lhs,
                            const difference_t &rhs) noexcept {
  return index_t(make_unsigned(make_signed(get(lhs)) - get(rhs)));
}
...
/// \module types
constexpr difference_t operator-(const index_t &lhs,
                                 const index_t &rhs) noexcept {
...
template <typename Indexable>
auto index_valid(non_member_size, const Indexable &obj,
                 const std::size_t &index) -> decltype(index < size(obj)) {
...
/// \returns The `i`th element of `obj` by invoking its `operator[]` with the
/// [ts::index_t]() converted to `std::size_t`. \requires `index` must be a
/// valid index for `obj`, i.e. less than the size of `obj`. \exclude return
/// \module types
template <typename Indexable>
auto at(Indexable &&obj, const index_t &index) -> decltype(
    std::forward<Indexable>(obj)[static_cast<std::size_t>(get(index))]) {
...
/// \notes This is the same as `index += dist` and the equivalent of
/// [std::advance()](). \module types
inline void advance(index_t &index, const difference_t &dist) { index += dist; }

/// \returns The distance between two [ts::index_t]() objects,
/// i.e. how often you'd have to increment `a` to reach `b`.
/// \notes This is the same as `b - a` and the equivalent of
/// [std::distance()](). \module types
...
/// \notes This is the same as `index + dist` and the equivalent of
/// [std::next()](). \module types
constexpr index_t next(const index_t &index,
                       const difference_t &dist = difference_t(1)) {
  return index + dist;
}

/// \returns The [ts::index_t]() that is `dist` smaller than `index`.
/// \notes This is the same as `index - dist` and the equivalent of
/// [std::prev()](). \module types
constexpr index_t prev(const index_t &index,
                       const difference_t &dist = difference_t(1)) {
  return index - dist;
}
} // namespace type_safe
