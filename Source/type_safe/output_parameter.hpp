...
///
/// An output parameter is a parameter that will be used to transport output of
/// a function to its caller, like a return value does. Usually they are
/// implemented with lvalue references. They have a couple of disadvantages
/// though:
/// * They require an already created object, i.e. a default constructor or
/// similar.
/// * It is not obvious when just looking at the call that the argument will be
/// changed.
/// * They make it easy to accidentally use them as regular parameters, i.e.
/// reading their value, even though that was not intended.
///
...
template <typename T> class output_parameter {
public:
  ...
  /// \requires The referred object must live as long as the function has not
  /// returned.
  explicit output_parameter(T &obj) noexcept
      : ptr_(&obj), is_normal_ptr_(true) {}
  ...
  /// All output will be assigned or created in the storage of the defer
  /// construction object, depending on wheter it is initialized. \requires The
  /// referred object must live as long as the function has not returned.
  ...
  /// \notes This constructor is only there because guaranteed copy elision
  /// isn't available and otherwise the `out()` function could not be
  /// implemented. It is not intended to use it otherwise.
  ...
  /// \returns A reference to the value that was assigned, *not* `*this` as
  /// normal "assignment" operators. \requires `value_type` must be
  /// constructible from `U`. \synopsis_return parameter_type&
  template <typename U>
  auto operator=(U &&u) -> typename std::enable_if<
      std::is_constructible<T, decltype(std::forward<U>(u))>::value,
      parameter_type &>::type {
    return assign(std::forward<U>(u));
  }

  /// \effects If the output object is not already created, forwards `args` to
  /// the constructor. Otherwise if `args` is a single type and can be assigned
  /// to `T`, assigns it. Otherwise if `args` cannot be assigned directly,
  /// creates a temporary object and move assigns that. \returns A reference to
  /// the value that was assigned. \throws Anything thrown by the constructor
  /// for `T` or the chosen assignment operator. \requires `T` must be
  /// constructible from the arguments,
  // and `T` must be move-assignable.
  template <typename... Args> T &assign(Args &&... args) {
    if (is_normal_ptr_)
...
private:
  ...
  template <typename... Args> void assign_impl(Args &&... args) {
...
template <typename T> output_parameter<T> out(T &obj) noexcept {
  return output_parameter<T>(obj);
}

/// \returns A new [ts::output_parameter]() using the
/// [ts::deferred_construction]() as output.
template <typename T>
