...
/// A `StoragePolicy` for [ts::basic_optional]() that is more space efficient
/// than [ts::direct_optional_storage]().
///
/// It is designed to have no space overhead compared to a regular object of the
/// stored type. This is accomplished by marking one value of the stored type as
/// invalid and using that in the empty state. What the invalid value is is
/// controlled by the `CompactPolicy`. It must provide the following `static`
...
/// * `storage_type invalid_value()` - returns the value that marks the optional
/// as empty
/// * `bool is_invalid(const storage_type&)` - returns `true` if the given value
...
/// The implementation assumes that `invalid_value()` and `is_invalid()` are
/// `noexcept` and cheap. \notes For a compact optional of pointer type, use
/// [ts::optional_ref](). \module optional
template <class CompactPolicy> class compact_optional_storage {
public:
  using value_type =
      typename std::remove_cv<typename CompactPolicy::value_type>::type;
  static_assert(
  ...
  using lvalue_reference =
      detail::storage_reference<storage_type, value_type, value_type &>;
  ...
  /// \throws Anything thrown by the constructor of `value_type`/`storage_type`
  /// or its move assignment operator in which case `has_value()` is still
  /// `false`. \requires `has_value() == false` and the given value must not be
  /// invalid. \notes This function does not participate in overload resolution
  /// unless `value_type` is constructible from `args`. \synopsis template
  ...
  auto create_value(Args &&... args) -> typename std::enable_if<
    ...
    storage_ =
        static_cast<storage_type>(value_type(std::forward<Args>(args)...));
  ...
  void destroy_value() noexcept { storage_ = CompactPolicy::invalid_value(); }
...
/// An alias for [ts::basic_optional]() using [ts::compact_optional_storage]()
/// with the given `CompactPolicy`. \module optional
template <class CompactPolicy>
using compact_optional =
    basic_optional<compact_optional_storage<CompactPolicy>>;

/// A `CompactPolicy` for [ts::compact_optional_storage]() for boolean types.
///
/// It is designed for either `bool` or [ts::boolean]().
/// \notes It uses a different `storage_type` and thus cannot return a reference
/// to the stored value. \module optional
template <typename Boolean> class compact_bool_policy {
public:
  using value_type = Boolean;
  using storage_type = char;

  static storage_type invalid_value() noexcept { return 3; }
...
template <typename Integer, Integer Invalid> class compact_integer_policy {
...
public:
  ...
  static storage_type invalid_value() noexcept { return Invalid; }
...
/// A `CompactPolicy` for [ts::compact_optional_storage]() for floating point
/// types.
///
/// `NaN` will be used to mark an empty optional.
/// \module optional
template <typename FloatingPoint> class compact_floating_point_policy {
  static_assert(std::is_floating_point<FloatingPoint>::value,
...
template <typename Enum> struct underlying_type_impl {
...
/// A `CompactPolicy` for [ts::compact_optional_storage]() for enumeration
/// types.
///
/// It uses the given `Invalid` value of the underlying type to mark an empty
/// optional. \notes It uses a different `storage_type` and thus cannot return a
/// reference to the stored value. \module optional
...
///
/// A `Container` is a type with a cheap no-throwing default constructor
/// initializing it empty, and either an `empty()` member function or ADL
/// function that returns `true` if the container is empty, `false` otherwise.
/// An empty container will be marked as an empty optional.
/// \module optional
template <class Container> class compact_container_policy {
public:
  using value_type = Container;
  using storage_type = Container;

  static storage_type invalid_value() noexcept { return {}; }
