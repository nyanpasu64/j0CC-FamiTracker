...
///
/// For each enum that should be used with [ts::flag_set]() it must provide the
/// following interface:
/// * Inherit from [std::true_type]().
/// * `static constexpr std::size_t size()` that returns the number of
/// enumerators.
///
/// The default specialization automatically works for enums that have an
/// enumerator `_flag_set_size`, whose value is the number of enumerators. But
/// you can also specialize the traits for your own enums. Enums which work with
/// [ts::flag_set]() are called flags.
///
/// \requires For all specializations the enum must be contiguous starting at
/// `0`, simply don't set an explicit value to the enumerators.
template <typename Enum> struct flag_set_traits : detail::is_flag_set<Enum> {
...
/// \exclude
namespace detail {
template <std::size_t Size, typename = void> struct select_flag_set_int {
  static_assert(
      Size != 0u,
...
#define TYPE_SAFE_DETAIL_SELECT(Min, Max, Type)                                \
  ...
  struct select_flag_set_int<                                                  \
      Size, typename std::enable_if<(Size > Min && Size <= Max)>::type> {      \
    using type = Type;                                                         \
  };
...
TYPE_SAFE_DETAIL_SELECT(32u, sizeof(std::uint_least64_t) * CHAR_BIT,
                        std::uint_least64_t)

#undef TYPE_SAFE_DETAIL_SELECT

template <typename Enum, typename Tag = void> class flag_set_impl {
public:
  ...
  static constexpr flag_set_impl all_set() {
    return flag_set_impl(
        int_type((int_type(1) << traits::size()) - int_type(1)));
  }
  ...
  constexpr int_type to_int() const { return bits_; }
...
private:
  ...
  template <typename Enum2, typename Tag2> friend class flag_set_impl;
};

template <typename Enum>
using flag_combo = flag_set_impl<Enum, struct combo_tag>;

template <typename Enum> using flag_mask = flag_set_impl<Enum, struct mask_tag>;
...
template <typename Enum>
constexpr bool operator==(const flag_combo<Enum> &a,
                          const flag_combo<Enum> &b) {
...
template <typename Enum>
constexpr flag_combo<Enum> operator|(const flag_combo<Enum> &a,
                                     const flag_combo<Enum> &b) {
...
template <typename Enum>
constexpr flag_mask<Enum> operator&(const flag_mask<Enum> &a,
                                    const flag_mask<Enum> &b) {
...
template <typename T, typename Enum> struct is_flag_combo : std::false_type {};
...
using enable_flag_combo =
    typename std::enable_if<is_flag_combo<T, Enum>::value>::type;
...
template <typename Enum> using flag_combo = detail::flag_combo<Enum>;
...
/// \requires `Enum` must be a flag,
/// i.e. valid with the [ts::flag_set_traits]().
template <typename Enum> using flag_mask = detail::flag_mask<Enum>;
...
///
/// Unlike [ts::flag_combo]() or [ts::flag_mask]() this does not have this
/// semantic distinction. It is just a generic container of flags, which can be
/// set, cleared or toggled. It can be interpreted as either a flag combination
/// or flag mask, however.
///
/// \requires `Enum` must be a flag,
/// i.e. valid with the [ts::flag_set_traits]().
template <typename Enum> class flag_set {
...
public:
  ...
  constexpr flag_set() noexcept
      : flags_(detail::flag_set_impl<Enum>::none_set()) {}

  /// \group ctor_null
  constexpr flag_set(noflag_t) noexcept : flag_set() {}

  /// \effects Creates a set where all bits are set to `0` except the given
  /// ones. \notes This constructor only participates in overload resolution if
  /// the argument is a flag combination.
  template <typename FlagCombo,
  ...
  /// \notes (2) does not participate in overload resolution unless `T` is a
  /// boolean-like type. \group set
  void set(const Enum &flag) noexcept { flags_ = flags_.set(flag); }
  ...
  /// \group set
  void set(const Enum &f, flag value) noexcept { set(f, value == true); }

  /// \effects Sets the specified flag to `0`.
  void reset(const Enum &flag) noexcept { flags_ = flags_.reset(flag); }

  /// \effects Toggles the specified flag.
  void toggle(const Enum &flag) noexcept { flags_ = flags_.toggle(flag); }

  /// \effects Sets/resets/toggles all flags.
  /// \group all
  void set_all() noexcept { flags_ = flags_.all_set(); }
  ...
  /// \group all
  void set_all(flag value) noexcept { set_all(value == true); }

  /// \group all
  void reset_all() noexcept { flags_ = flags_.none_set(); }

  /// \group all
  void toggle_all() noexcept { flags_ = flags_.toggle_all(); }
  ...
  /// \returns An integer where each bit has the value of the corresponding
  /// flag. \requires `T` must be an unsigned integer type with enough bits.
  template <typename T> constexpr T to_int() const noexcept {
...
/// \returns Whether both flag sets have the same combination of flags set/not
/// set. \group flag_set_equal flag_set equality comparison
template <typename Enum>
constexpr bool operator==(const flag_set<Enum> &a,
...
template <typename Enum, typename FlagCombo,
          typename = detail::enable_flag_combo<FlagCombo, Enum>>
constexpr bool operator==(const FlagCombo &a,
                          const flag_set<Enum> &b) noexcept {
  return a == combo(b);
}
...
template <typename Enum, typename FlagCombo,
          typename = detail::enable_flag_combo<FlagCombo, Enum>>
constexpr flag_set<Enum> operator|(const flag_set<Enum> &a,
                                   const FlagCombo &b) {
...
template <typename Enum, typename FlagCombo,
          typename = detail::enable_flag_combo<FlagCombo, Enum>>
constexpr flag_set<Enum> operator|(const FlagCombo &a,
                                   const flag_set<Enum> &b) {
  return b | a;
}
/// \group bitwise_op
/// \param 2
...
/// \group bitwise_op
template <typename Enum>
constexpr flag_set<Enum> operator&(const flag_mask<Enum> &a,
                                   const flag_set<Enum> &b) {
  return b & a;
}
...
/// unless `Enum` is an `enum` where the [ts::flag_set_traits]() are
/// specialized. \param 1 \exclude
...
/// specialized. \param 1 \exclude
template <typename Enum, typename = type_safe::detail::enable_flag<Enum>>
constexpr type_safe::flag_combo<Enum> operator|(const Enum &a,
                                                const Enum &b) noexcept {
