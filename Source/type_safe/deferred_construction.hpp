...
/// yet it is not as flexible and does not allow to reset it to the
/// uninitialized state, once initialized.
template <typename T> class deferred_construction {
public:
  using value_type = T;
  ...
  /// \throws Anything thrown by the copy constructor of `value_type` if `other`
  /// is initialized.
  ...
  /// \effects If `other` is un-initialized, it will be un-initialized as well.
  /// If `other` is initialized, it will copy the stored value.
  /// \throws Anything thrown by the move constructor of `value_type` if `other`
  /// is initialized. \notes `other` will still be initialized after the move
  /// operation, it is just in a moved-from state.
  ...
  //=== modifiers ===//
  /// \effects Initializes the object with the `value_type` constructed from
  /// `args`. \requires `has_value() == false`. \throws Anything thrown by the
  /// chosen constructor of `value_type`. \notes You must only call this
  /// function once, after the object has been initialized, you can use
  /// `value()` to assign to it. \output_section Modifiers
