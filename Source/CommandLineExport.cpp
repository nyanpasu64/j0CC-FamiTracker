...
public:
  ...
  void WriteLog(LPCTSTR text) { m_pFile->WriteString(text); };
  void Clear(){};
...
void CCommandLineExport::CommandLineExport(const CString &fileIn,
                                           const CString &fileOut,
                                           const CString &fileLog,
                                           const CString &fileDPCM) {
  // open log
  bool bLog = false;
  CStdioFile fLog;
  if (fileLog.GetLength() > 0) {
    if (fLog.Open(fileLog,
                  CFile::modeCreate | CFile::modeWrite | CFile::typeText, NULL))
      bLog = true;
  }
  ...
  theApp.GetSoundGenerator()->GenerateVibratoTable(
      pExportDoc->GetVibratoStyle());
  ...
  } else // use first custom exporter
  {
    ...
    if (pExporters) {
      ...
      if (sNames.GetCount()) {
        ...
        bool bResult = (pExporters->GetCurrentExporter().Export(
            &documentWrapper, fileOut));
        if (bLog) {
  ...
  if (bLog) {
    fLog.WriteString(
        _T("Error: unable to find matching export extension for: "));
    fLog.WriteString(fileOut);
    fLog.WriteString(_T("\n"));
  }
  return;
