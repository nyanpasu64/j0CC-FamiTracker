...
public:
  ...
  void AddProduct(Key Index, Arg &&... x);
...
template <typename Key, typename Base> inline CFactory<Key, Base>::CFactory() {}
...
template <typename Key, typename Base>
template <typename T, typename... Arg>
inline typename CFactory<Key, Base>::FuncType
CFactory<Key, Base>::MakeCtor(Arg &&... x) {
  return std::make_shared<std::function<Base *()>>(
      std::bind([](auto &&... y) -> Base * { return new T{y...}; },
                std::forward<Arg>(x)...));
}
