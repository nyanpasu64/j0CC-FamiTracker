...
CInstrumentManager::CInstrumentManager(CFTMComponentInterface *pInterface)
    : m_pDSampleManager(new CDSampleManager()), m_pInstruments(MAX_INSTRUMENTS),
      m_pDocInterface(pInterface) {
  for (int i = 0; i < SEQ_MANAGER_COUNT; i++)
    m_pSequenceManager.push_back(std::unique_ptr<CSequenceManager>(
        new CSequenceManager(i == 2 ? 3 : SEQ_COUNT)));
}
...
CInstrumentManager::GetInstrument(unsigned int Index) const {
...
std::shared_ptr<CInstrument>
CInstrumentManager::CreateNew(inst_type_t InstType) {
  return std::shared_ptr<CInstrument>(CInstrumentFactory::CreateNew(InstType));
}

bool CInstrumentManager::InsertInstrument(unsigned int Index,
                                          CInstrument *pInst) {
...
inst_type_t CInstrumentManager::GetInstrumentType(unsigned int Index) const {
  return !IsInstrumentUsed(Index) ? INST_NONE
                                  : m_pInstruments[Index]->GetType();
}

void CInstrumentManager::CloneInstrumentShallow(unsigned int Old,
                                                unsigned int New) {}

void CInstrumentManager::CloneInstrumentDeep(unsigned int Old,
                                             unsigned int New) {}
...
CInstrumentManager::GetSequenceManager(int InstType) const {
  int Index = -1;
  switch (InstType) {
  case INST_2A03:
    Index = 0;
...
CSequence *CInstrumentManager::GetSequence(int InstType, int SeqType,
                                           int Index) const {
...
void CInstrumentManager::SetSequence(int InstType, int SeqType, int Index,
                                     CSequence *pSeq) {
...
int CInstrumentManager::AddSequence(int InstType, int SeqType, CSequence *pSeq,
                                    CSeqInstrument *pInst) {
  int Index =
      GetFreeSequenceIndex(static_cast<inst_type_t>(InstType), SeqType, pInst);
  if (Index != -1)
