...
/**
        \brief Indices for the types supported by the waveform generator
   parameters.
*/
...
protected:
  /** \brief Constructor of the waveform generator parameter.
          \param Type The type of the parameter. Provided in the constructor of
     derived classes. \param Name The name of the parameter. */
...
public:
  ...
  /** \brief Sets the value of the parameter.
          \details There is no corresponding getter; it is only required in the
     generator editor. \return True if the operation succeeded, false if it
     failed. */
...
/**
        \brief Unsigned integer specialization of the waveform generator
   parameter.
*/
...
/**
        \brief Floating-point value specialization of the waveform generator
   parameter.
*/
...
public:
  ...
  /** \brief Generates floating-point waveforms.
          \details Successive floating-point values are written to an array for
     further quantization. The waveforms should be expected to have a range
     between -1 and 1. \param Dest Pointer to a floating-point array. \param
     Size The number of samples per waveform. \param Index The waveform number
     to generate, or -1 to generate all waveforms consecutively.
          \return True if the operation succeeded, false if it failed. */
  virtual bool CreateWaves(float *const Dest, unsigned int Size,
                           unsigned int Index) = 0;
  /** \brief Obtains a parameter object.
          \details Out-of-bound indices \b must return nullptr to signal the end
     of the waveform generator's parameter list. \param Index A unique numerical
     index. \return Pointer to a parameter of the waveform generator. */
  virtual CWavegenParam *GetParameter(unsigned int Index) const = 0;
  /** \brief Obtains the expected number of waves generated.
          \details This may depend on the waveform generator's own parameter
     list. \return The wave count of the waveform generator. */
  virtual unsigned int GetCount() const = 0;
  /** \brief Returns a status describing the last call to CreateWaves.
          \details This function should return an empty string if no message is
     to be displayed on. success. If waveform generation fails, the generator
     dialog shows this message and prevents the user from exiting the dialog.
