...
const TCHAR *CDetuneDlg::m_pNote[12] = {_T("C"),  _T("C#"), _T("D"),  _T("D#"),
                                        _T("E"),  _T("F"),  _T("F#"), _T("G"),
                                        _T("G#"), _T("A"),  _T("A#"), _T("B")};
const TCHAR *CDetuneDlg::m_pNoteFlat[12] = {
    _T("C"),  _T("D-"), _T("D"),  _T("E-"), _T("E"),  _T("F"),
    _T("G-"), _T("G"),  _T("A-"), _T("A"),  _T("B-"), _T("B")};
const CString CDetuneDlg::CHIP_STR[6] = {_T("NTSC"), _T("PAL"), _T("Saw"),
                                         _T("VRC7"), _T("FDS"), _T("N163")};
...
BOOL CDetuneDlg::OnInitDialog() {
  ...
  auto pSlider =
      static_cast<CSliderCtrl *>(GetDlgItem(IDC_SLIDER_DETUNE_SEMITONE));
...
const int *CDetuneDlg::GetDetuneTable() const { return *m_iDetuneTable; }

int CDetuneDlg::GetDetuneSemitone() const { return m_iGlobalSemitone; }

int CDetuneDlg::GetDetuneCent() const { return m_iGlobalCent; }
...
unsigned int CDetuneDlg::FreqToReg(double Freq, int Chip, int Octave) {
  ...
  default:
  case 0:
    dReg = BASE_FREQ_NTSC / Freq / 16.0 - 1.0;
    break;
  case 1:
    dReg = BASE_FREQ_PAL / Freq / 16.0 - 1.0;
    break;
  case 2:
    dReg = BASE_FREQ_NTSC / Freq / 14.0 - 1.0;
    break;
  case 3:
    dReg = Freq / 49716.0 * (1 << (18 - Octave));
    break;
  case 4:
    dReg = Freq / BASE_FREQ_NTSC * (1 << 20);
    break;
  case 5:
    dReg = Freq / BASE_FREQ_NTSC * 15.0 * (1 << 18) *
           m_pDocument->GetNamcoChannels();
    break;
  }
...
double CDetuneDlg::RegToFreq(unsigned int Reg, int Chip, int Octave) {
  ...
  default:
  case 0:
    return BASE_FREQ_NTSC / 16.0 / (Reg + 1.0);
    break;
  case 1:
    return BASE_FREQ_PAL / 16.0 / (Reg + 1.0);
    break;
  case 2:
    return BASE_FREQ_NTSC / 14.0 / (Reg + 1.0);
    break;
  case 3:
    return 49716.0 * Reg / (1 << (18 - Octave));
    break;
  case 4:
    return BASE_FREQ_NTSC * Reg / (1 << 20);
    break;
  case 5:
    return BASE_FREQ_NTSC * Reg / 15 / (1 << 18) /
           m_pDocument->GetNamcoChannels();
    break;
  }
}
...
void CDetuneDlg::UpdateOffset() {
  ...
  if (m_iCurrentChip == 3) // VRC7
    for (int i = 0; i < OCTAVE_RANGE; i++)
      m_iDetuneTable[3][i * NOTE_RANGE + m_iNote % NOTE_RANGE] =
          m_iDetuneTable[3][m_iNote];
  ...
  for (int i = 0; i < 6; i++) {
    ...
    int newReg = std::max(0, (int)oldReg + m_iDetuneTable[i][m_iNote] *
                                               (i >= 3 ? 1 : -1));
    double newFreq = RegToFreq(newReg, i, m_iNote / NOTE_RANGE);
    double values[4] = {
        RegToFreq(oldReg, i, m_iNote / NOTE_RANGE) * (i == 4 ? .25 : 1),
        newFreq * (i == 4 ? .25 : 1), NoteToFreq(Note) * (i == 4 ? .25 : 1),
        1200.0 * log(newFreq / NoteToFreq(Note)) / log(2.0)};
    for (const auto x : values)
      fmt += DoubleFunc(x);

    str.Format(fmt, CHIP_STR[i], oldReg, newReg, values[0], values[1],
               values[2], values[3]);
...
void CDetuneDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar) {
  ...
  if (pScrollBar == (CScrollBar *)m_cSliderOctave) {
    ...
    m_iGlobalCent =
        static_cast<CSliderCtrl *>(GetDlgItem(IDC_SLIDER_DETUNE_CENT))
            ->GetPos();
    UpdateOffset();
  }
}
...
void CDetuneDlg::OnBnClickedButtonImport() {
  ...
  CFileDialog FileDialog(
      TRUE, _T("csv"), 0, OFN_HIDEREADONLY,
...
void CDetuneDlg::OnBnClickedButtonExport() {
  ...
  CFileDialog SaveFileDialog(
      FALSE, _T("csv"), (LPCSTR)m_pDocument->GetFileTitle(),
