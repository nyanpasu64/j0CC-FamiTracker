...
unsigned CBookmarkCollection::GetCount() const { return m_pBookmark.size(); }
...
bool CBookmarkCollection::SetBookmark(unsigned Index, CBookmark *const pMark) {
  if (m_pBookmark[Index].get()->IsEqual(*pMark))
    return false;
  m_pBookmark[Index].reset(pMark);
  return true;
}

bool CBookmarkCollection::InsertBookmark(unsigned Index,
                                         CBookmark *const pMark) {
  if (Index > m_pBookmark.size())
    return false;
  m_pBookmark.insert(m_pBookmark.begin() + Index,
                     std::unique_ptr<CBookmark>(pMark));
  return true;
}
...
void CBookmarkCollection::RemoveFrames(unsigned Frame, unsigned Count) {
  m_pBookmark.erase(std::remove_if(m_pBookmark.begin(), m_pBookmark.end(),
                                   [&](std::unique_ptr<CBookmark> &a) {
                                     return a->m_iFrame >= Frame &&
                                            a->m_iFrame < Frame + Count;
                                   }),
                    m_pBookmark.end());
  std::for_each(m_pBookmark.begin(), m_pBookmark.end(),
                [&](std::unique_ptr<CBookmark> &a) {
...
CBookmark *CBookmarkCollection::FindAt(unsigned Frame, unsigned Row) const {
  ...
  auto it = std::find_if(
      m_pBookmark.begin(), m_pBookmark.end(),
...
void CBookmarkCollection::RemoveAt(unsigned Frame, unsigned Row) {
  const CBookmark tmp(Frame, Row);
  m_pBookmark.erase(std::remove_if(m_pBookmark.begin(), m_pBookmark.end(),
                                   [&](const std::unique_ptr<CBookmark> &a) {
                                     return *a.get() == tmp;
                                   }),
                    m_pBookmark.end());
}
...
CBookmark *CBookmarkCollection::FindNext(unsigned Frame, unsigned Row) const {
  ...
  return std::min_element(m_pBookmark.begin(), m_pBookmark.end(),
                          [&](const std::unique_ptr<CBookmark> &a,
                              const std::unique_ptr<CBookmark> &b) {
                            return static_cast<unsigned>(a->Distance(temp) -
                                                         1) <
                                   static_cast<unsigned>(b->Distance(temp) - 1);
                          })
      ->get();
}

CBookmark *CBookmarkCollection::FindPrevious(unsigned Frame,
                                             unsigned Row) const {
  if (m_pBookmark.empty())
    return nullptr;
  CBookmark temp(Frame, Row);
  return std::min_element(m_pBookmark.begin(), m_pBookmark.end(),
                          [&](const std::unique_ptr<CBookmark> &a,
                              const std::unique_ptr<CBookmark> &b) {
                            return static_cast<unsigned>(temp.Distance(*a) -
                                                         1) <
                                   static_cast<unsigned>(temp.Distance(*b) - 1);
                          })
      ->get();
}

bool CBookmarkCollection::SortByName(bool Desc) {
  static auto sortFunc = [](const std::unique_ptr<CBookmark> &a,
                            const std::unique_ptr<CBookmark> &b) {
  ...
  bool Change =
      !std::is_sorted(m_pBookmark.begin(), m_pBookmark.end(), sortFunc);
  if (Desc)
    std::reverse(m_pBookmark.begin(), m_pBookmark.end());
  if (!Change)
    return false;
