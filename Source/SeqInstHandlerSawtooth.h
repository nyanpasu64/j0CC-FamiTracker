...
/*!
        ...
        \details The sawtooth channel adds special processing for both 16-step
   and 64-step volume sequences.
*/
...
public:
   ...
   */
  CSeqInstHandlerSawtooth(CChannelHandlerInterface *pInterface, int Vol,
                          int Duty)
      : CSeqInstHandler(pInterface, Vol, Duty) {}

  /*!	\brief Starts a new note for the instrument handler.
          \details This reimplementation checks whether the current instrument
     uses a 64-step volume sequence. */
  void TriggerInstrument() override;

  /*!	\brief Queries whether the duty sequence should be ignored when
     calculating the volume. \return Whether the current instrument uses a
     64-step volume sequence. */
