...
// TODO replace above with std::string, and don't subtract std::size()-null
// terminator
...
constexpr int LowPercent{80}, HiPercent{125}, StepPercent{5},
...
void CConfigAppearance::OnBnClickedButtonAppearanceSave() // // // 050B
{
  ...
  CFileDialog fileDialog{FALSE, TXT, _T("Theme.txt"),
...
void CConfigAppearance::OnBnClickedButtonAppearanceLoad() // // // 050B
{
  ...
  if (fileDialog.DoModal() == IDOK) {
    ...
    static_cast<CComboBox *>(GetDlgItem(IDC_FONT_SIZE))
        ->SelectString(0, MakeIntString(m_rowHeight));
    this->fontPercentList.SelectString(0, MakeIntString(fontPercent));
    RedrawWindow();
    SetModified();
  }
...
void CConfigAppearance::ExportSettings(const char *Path) const // // // 050B
{
  if (auto file = std::fstream{Path, std::ios_base::out}) {
    ...
    file << "Font percent" << SETTING_SEPARATOR << this->fontPercent
         << std::endl;
  }
}

const std::regex SETTING_REGEX("([^:]+)"s +
                               CConfigAppearance::SETTING_SEPARATOR + "(.*)");
// [0, 1, 2]
...
std::string_view rtrim(std::string_view &str) {
  ...
  auto rightIt = std::find_if(str.rbegin(), str.rend(), [](char ch) {
    return !std::isspace<char>(ch, std::locale::classic());
  });
  ...
  while (true) {
    ...
    for (size_t i = 0; i < sizeof(m_iColors) / sizeof(*m_iColors); ++i) {
      ...
      if (n == std::string_view::npos)
        continue;
      if (auto c =
              conv::to_uint32(value.substr(n + std::size(HEX_PREFIX) - 1), 16))
        m_iColors[i] = *c;
    }
