...
void CInstHandlerVRC7::TriggerInstrument() { UpdateRegs(); }
...
void CInstHandlerVRC7::UpdateInstrument() {
  if (!m_bUpdate)
    return;
  CChannelHandlerInterfaceVRC7 *pInterface =
      dynamic_cast<CChannelHandlerInterfaceVRC7 *>(m_pInterface);
  if (pInterface == nullptr)
    return;
  auto pVRC7Inst =
      std::dynamic_pointer_cast<const CInstrumentVRC7>(m_pInstrument);
  if (pVRC7Inst == nullptr)
    return;
...
void CInstHandlerVRC7::UpdateRegs() { m_bUpdate = true; }
