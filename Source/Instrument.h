...
#pragma once
#pragma warning(disable : 4351) // // // "new behaviour: elements of array [...]
...
public:
  CInstrumentFile(LPCTSTR lpszFileName, UINT nOpenFlags)
...
public:
  ...
  virtual void
  Store(CDocumentFile *pDocFile) = 0; // Saves the instrument to the module
  virtual bool
  Load(CDocumentFile *pDocFile) = 0; // Loads the instrument from a module
  virtual void SaveFile(CInstrumentFile *pFile) = 0; // Saves to an FTI file
  virtual bool LoadFile(CInstrumentFile *pFile,
                        int iVersion) = 0; // Loads from an FTI file
  virtual int
  Compile(CChunk *pChunk,
          int Index) = 0; // // // Compiles the instrument for NSF generation
