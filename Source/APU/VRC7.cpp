...
const float CVRC7::AMPLIFY =
    4.6f; // Mixing amplification, VRC7 patch 14 is 4,88 times stronger than a
          // 50% square @ v=15
const uint32_t CVRC7::OPL_CLOCK = 3579545; // Clock frequency

CVRC7::CVRC7(CMixer *pMixer)
    : CSoundChip(pMixer), m_pBuffer(NULL), m_pOPLLInt(NULL), m_fVolume(1.0f),
      m_iMaxSamples(0), m_iSoundReg(0) {
...
void CVRC7::SetSampleSpeed(uint32_t SampleRate, double ClockRate,
                           uint32_t FrameRate) {
  if (m_pOPLLInt != NULL) {
    OPLL_delete(m_pOPLLInt);
  }
...
void CVRC7::SetVolume(float Volume) { m_fVolume = Volume * AMPLIFY; }
...
uint8_t CVRC7::Read(uint16_t Address, bool &Mapped) { return 0; }
