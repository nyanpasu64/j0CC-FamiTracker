...
/*!
        ...
        \details This class implements common facilities for manipulating a
   fixed-length array of instrument objects.
*/
...
public:
  ...
  int GetFreeSequenceIndex(inst_type_t InstType, int Type,
  ...
  // from interface
  CSequence *GetSequence(int InstType, int SeqType, int Index)
      const; // TODO: use SetSequence and provide const getter
  void SetSequence(int InstType, int SeqType, int Index, CSequence *pSeq);
  int AddSequence(int InstType, int SeqType, CSequence *pSeq,
                  CSeqInstrument *pInst = nullptr);
