...
CBookmark::CBookmark(unsigned Frame, unsigned Row)
    : m_iFrame(Frame), m_iRow(Row), m_bPersist(false), m_sName("Bookmark") {
  m_Highlight.First = 4;
...
unsigned CBookmark::Distance(const CBookmark &other) const {
  ...
  int Dist = ((m_iFrame - other.m_iFrame) * MAX_PATTERN_LENGTH + m_iRow -
              other.m_iRow) %
             ALL_ROWS;
  if (Dist < 0)
    Dist += ALL_ROWS;
...
bool CBookmark::IsEqual(const CBookmark &other) const {
  return *this == other && m_bPersist == other.m_bPersist &&
