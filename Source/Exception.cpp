...
static CString GetDumpFilename(int counter) {
  ...
  // Date
  filename.AppendFormat(_T("_%02i%02i%02i-%02i%02i"), t.GetYear(), t.GetMonth(),
...
static LONG WINAPI ExceptionHandler(__in struct _EXCEPTION_POINTERS *ep) {
  ...
  HANDLE hFile = CreateFile(MinidumpFile, GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  if ((hFile != NULL) && (hFile != INVALID_HANDLE_VALUE)) {
    ...
    MiniDumpWriteDump(hProcess, dwProcessId, hFile, MiniDumpNormal,
                      (ep != 0) ? &mdei : NULL, 0, 0);
  ...
  CString text;
  text.Format(
      _T("This application has encountered a problem and needs to close.\n\n"));
  text.AppendFormat(_T("Unhandled exception %X.\n\n"),
                    ep->ExceptionRecord->ExceptionCode);
  text.AppendFormat(_T("A memory dump file has been created (%s), please ")
                    _T("include this if you file a bug report!\n\n"),
                    LPCTSTR(MinidumpFile));
  text.AppendFormat(_T("Attempting to save current module as %s."),
                    LPCTSTR(DocDumpFile));
...
void UninstallExceptionHandler() { SetUnhandledExceptionFilter(NULL); }
