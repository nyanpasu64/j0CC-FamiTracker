...
CSeqInstrument::CSeqInstrument(inst_type_t type)
    : CInstrument(type), m_iSeqEnable(), m_iSeqIndex() {}
...
bool CSeqInstrument::Load(CDocumentFile *pDocFile) {
  int SeqCnt = CModuleException::AssertRangeFmt(
      pDocFile->GetBlockInt(), 0, SEQ_COUNT, "Instrument sequence count", "%i");
  SeqCnt = SEQ_COUNT;
  ...
  for (int i = 0; i < SeqCnt; ++i) {
    ...
    SetSeqIndex(i, CModuleException::AssertRangeFmt(Index, 0, MAX_SEQUENCES - 1,
                                                    "Instrument sequence index",
                                                    "%i"));
  }
...
bool CSeqInstrument::LoadFile(CInstrumentFile *pFile, int iVersion) {
  ...
  unsigned char SeqCount = CModuleException::AssertRangeFmt(
      pFile->ReadChar(), 0, SEQ_COUNT, "Sequence count", "%i");
  ...
  for (unsigned i = 0; i < SeqCount; ++i)
    try {
      ...
      // Read the sequence
      int Count = CModuleException::AssertRangeFmt(pFile->ReadInt(), 0U, 0xFFU,
                                                   "Sequence item count", "%i");
      ...
      if (iVersion < 20) {
        ...
        pSeq->SetLoopPoint(CModuleException::AssertRangeFmt(
            static_cast<int>(pFile->ReadInt()), -1, Count2 - 1,
            "Sequence loop point", "%i"));
        if (iVersion > 20) {
          pSeq->SetReleasePoint(CModuleException::AssertRangeFmt(
              static_cast<int>(pFile->ReadInt()), -1, Count2 - 1,
              "Sequence release point", "%i"));
          if (iVersion >= 22)
...
int CSeqInstrument::Compile(CChunk *pChunk, int Index) {
  ...
  case INST_2A03:
    pChunk->StoreByte(0);
    label = CChunkRenderText::LABEL_SEQ_2A03;
    break;
  case INST_VRC6:
    pChunk->StoreByte(4);
    label = CChunkRenderText::LABEL_SEQ_VRC6;
    break;
  case INST_N163:
    pChunk->StoreByte(9);
    label = CChunkRenderText::LABEL_SEQ_N163;
    break;
  case INST_S5B:
    pChunk->StoreByte(10);
    label = CChunkRenderText::LABEL_SEQ_S5B;
    break;
  }
  ...
  for (unsigned i = 0; i < SEQ_COUNT; ++i) {
    ...
    if (GetSeqEnable(i) && pSequence != nullptr &&
        pSequence->GetItemCount() > 0)
      ModSwitch |= 1 << i;
  }
...
bool CSeqInstrument::CanRelease() const {
  return GetSeqEnable(SEQ_VOLUME) != 0 &&
         GetSequence(SEQ_VOLUME)->GetReleasePoint() != -1;
}
