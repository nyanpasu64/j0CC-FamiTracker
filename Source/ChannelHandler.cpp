...
CChannelHandler::CChannelHandler(int MaxPeriod, int MaxVolume)
    : m_iChannelID(0), m_iInstTypeCurrent(INST_NONE), // // //
      m_iInstrument(0), m_pNoteLookupTable(nullptr), m_pVibratoTable(nullptr),
      m_pAPU(nullptr), m_pInstHandler(), // // //
...
CString CChannelHandler::GetSlideEffectString() const // // //
{
  ...
  case EF_ARPEGGIO:
    if (m_iEffectParam)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect], m_iEffectParam);
    break;
  case EF_PORTA_UP:
  case EF_PORTA_DOWN:
  case EF_PORTAMENTO:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect], m_iPortaSpeed);
    break;
  }
...
void CChannelHandler::WriteRegister(uint16_t Reg, uint8_t Value) {
  ...
  m_pSoundGen->WriteRegister(
      Reg, Value); // This method is dead. Blame #ifdef WRITE_VGM.
}
...
int CChannelHandler::getDutyMax() const { return -1; }
...
bool FrequencyChannelHandler::HandleEffect(effect_t EffNum,
...
CString FrequencyChannelHandler::GetSlideEffectString() const // // //
{
  ...
  case EF_PORTA_UP:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_DOWN], m_iPortaSpeed);
    break;
  case EF_PORTA_DOWN:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_UP], m_iPortaSpeed);
    break;
  case EF_PORTAMENTO:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect], m_iPortaSpeed);
