...
public:
  ...
  void Update(uint8_t Val) {
    if (m_iValue != Val)
      m_iNewClock = DECAY_RATE;
    m_iValue = Val;
    m_iWriteClock = DECAY_RATE;
  }
  ...
  /*!	\brief Obtains the number of ticks since the last time the register
     value was updated. \return Number of elapsed ticks. */
  unsigned int GetLastUpdatedTime() const { return DECAY_RATE - m_iWriteClock; }

  /*!	\brief Obtains the number of ticks since the last time a new register
     value was written. \return Number of elapsed ticks. */
  unsigned int GetNewValueTime() const { return DECAY_RATE - m_iNewClock; }

  /*!	\brief Steps one tick and updates the register state's time
   * information. */
  void Step() {
    if (m_iWriteClock)
      --m_iWriteClock;
...
public:
  ...
  /*!	\brief Adds a range of register addresses, creating a state object for
     each register. \details The added addresses are not allowed to overlap
     previously added addresses. Each contiguous address range added in this way
     forms a region for the address port to wrap around when auto-incrementing
     after writes. \param Low The smallest address value. \param High The
     highest address value. \return Whether the registers were successfully
     added. */
  ...
  /*!	\brief Obtains a register object.
          ...
          \param The register state object, or nullptr if the given address does
     not exist. */
  CRegisterState *GetRegister(unsigned Address);

  /*!	\brief Steps one tick and updates the time information of all
   * registers. */
  void Step();
...
/*!
        \brief A class which allows internal changes to the register state
   without causing external changes due to logging. \details A register logger
   is blocked during the lifetime of a CRegisterLoggerBlock object. The address
   port of the logger before blocking remains after the block's destruction.
*/
