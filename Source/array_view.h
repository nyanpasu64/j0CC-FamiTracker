...
template <typename T> using data_func_t = decltype(std::declval<T>().data());
template <typename T> using size_func_t = decltype(std::declval<T>().size());
...
public:
  ...
  template <typename ValT, std::size_t N,
            std::enable_if_t<std::is_convertible_v<ValT *, const value_type *>,
                             int> = 0>
...
template <> class array_view<void>;
