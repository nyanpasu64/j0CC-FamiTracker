...
CSeqInstHandler::CSeqInstHandler(CChannelHandlerInterface *pInterface, int Vol,
                                 int Duty)
    : CInstHandler(pInterface, Vol), m_iDefaultDuty(Duty) {
...
void CSeqInstHandler::LoadInstrument(std::shared_ptr<CInstrument> pInst) {
  ...
  for (size_t i = 0; i < sizeof(m_pSequence) / sizeof(CSequence *); i++) {
    ...
    if (!Enable)
      ClearSequence(i);
    else if (pSequence != m_pSequence[i] ||
...
void CSeqInstHandler::ReleaseInstrument() {
  if (m_pInterface->IsReleasing())
    return;
  for (size_t i = 0; i < sizeof(m_pSequence) / sizeof(CInstrument *); i++)
    if (m_pSequence[i] != nullptr && (m_iSeqState[i] == SEQ_STATE_RUNNING ||
    ...
    case SEQ_STATE_RUNNING:
...

      {
        ...
        if (m_iSeqPointer[i] == (Release + 1) || m_iSeqPointer[i] >= Items) {
          // End point reached
          if (Loop != -1 && !(m_pInterface->IsReleasing() && Release != -1) &&
              Loop < Release) { // // //
        ...
        theApp.GetSoundGenerator()->SetSequencePlayPos(m_pSequence[i],
                                                       m_iSeqPointer[i]);
      }
      break;
    ...
    case SEQ_STATE_END:
      ...
      case SEQ_ARPEGGIO:
        if (m_pSequence[i]->GetSetting() == SETTING_ARP_FIXED)
          m_pInterface->SetPeriod(
              m_pInterface->TriggerNote(m_pInterface->GetNote()));
        break;
      }
...
bool CSeqInstHandler::ProcessSequence(int Index, unsigned Setting, int Value) {
  ...
  case SEQ_ARPEGGIO:
    ...
    case SETTING_ARP_ABSOLUTE:
      m_pInterface->SetPeriod(
          m_pInterface->TriggerNote(m_pInterface->GetNote() + Value));
      return true;
    ...
    case SETTING_ARP_SCHEME: // // //
      ...
      m_pInterface->SetPeriod(
          m_pInterface->TriggerNote(m_pInterface->GetNote() + lim));
      return true;
    }
    return false;
  // Pitch
