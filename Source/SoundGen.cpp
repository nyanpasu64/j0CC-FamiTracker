...
CSoundGen::CSoundGen()
    : m_pAPU(NULL), m_pDSound(NULL), m_pDSoundChannel(NULL),
      ...
      m_iPlayTicks(0), m_iConsumedCycles(0), m_iRegisterStream(), // // //
...
bool CSoundGen::ResetAudioDevice() {
  ...
  */
  ...
  TRACE("SoundGen: Created sound channel with params: %i Hz, %i bits, %i ms "
        "(%i blocks)\n",
...
static CString GetStateString(const stChannelState &State) {
  ...
  if (State.Effect_LengthCounter >= 0)
    effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_VOLUME],
                        State.Effect_LengthCounter);
  if (State.Effect_AutoFMMult >= 0)
    effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_FDS_MOD_DEPTH],
