...
/*!
        ...
        \details All sequences of a single instrument type and sequence type are
   grouped into
*/
...
public:
  ...
  /*!	\brief Obtains a modifiable sequence at a given index, creating the
     object if it does not exist. \param Index The index value of the sequence.
  ...
  */
  ...
  /*!	\brief Obtains a constant sequence at a given index.
          \param Index The index value of the sequence.
          \returns A pointer to the sequence object, or \b nullptr if it does
     not exist.
  */
  const CSequence *GetSequence(unsigned int Index) const;
  /*!	\brief Obtains a free sequence.
          \returns An index to the first sequence which has no entries, or -1 if
     none of the sequences is free.
  */
  ...
  /*!	\brief Obtains an unused sequence.
          ...
          \returns An index to the first sequence which has no entries and is
     not used by any sequence instrument, or -1 if all of the sequences are
     used.
  */
