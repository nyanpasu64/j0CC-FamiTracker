  ...
  for (int i = 0; i < MAX_GROOVE; i++)
    if (GrooveTable[i]->GetSize())
      ...
      for (unsigned j = 0; j < Tracks; j++)
        if (m_pDocument->GetSongSpeed(j) == i &&
            m_pDocument->GetSongGroove(j)) {
...
void CGrooveDlg::UpdateCurrentGroove() {
  ...
  String.Format(_T("%02X%s"), m_iGrooveIndex,
                Groove->GetSize() ? _T(" *") : _T(""));
...
void CGrooveDlg::UpdateIndicators() {
  ...
  String.Format(_T("Speed: %.3f"),
                Groove->GetSize() ? Groove->GetAverage() : DEFAULT_SPEED);
  SetDlgItemText(IDC_STATIC_GROOVE_AVERAGE, String);
  String.Format(_T("Size: %d bytes"),
                Groove->GetSize() ? Groove->GetSize() + 2 : 0);
...
void CGrooveDlg::OnBnClickedButtonGroovelDown() {
  if (m_iGrooveIndex == MAX_GROOVE - 1)
    return;
  CGroove *Temp = Groove;
  GrooveTable[m_iGrooveIndex] = GrooveTable[m_iGrooveIndex + 1];
  GrooveTable[m_iGrooveIndex + 1] = Temp;
...
void CGrooveDlg::OnBnClickedButtonGrooveCopyFxx() {
  ...
  CClipboard Clipboard(
      CFamiTrackerView::GetView(),
      ::RegisterClipboardFormat(CFamiTrackerView::CLIPBOARD_ID));
...
void CGrooveDlg::ParseGrooveField() {
  ...
  while (true) {
    ...
    m_iGroovePos++;
    if (m_iGroovePos == MAX_GROOVE_SIZE ||
        Str.FindOneOf(_T("0123456789")) == -1 || Str.Find(_T(' ')) == -1)
      break;
...
void CGrooveDlg::OnBnClickedButtonGrooveExpand() {
  if (Groove->GetSize() > MAX_GROOVE_SIZE / 2)
    return;
  for (int i = 0; i < Groove->GetSize(); i++)
    if (Groove->GetEntry(i) < 2)
      return;
  Groove->SetSize(Groove->GetSize() * 2);
  for (int i = Groove->GetSize() - 1; i >= 0; i--)
    Groove->SetEntry(i, Groove->GetEntry(i / 2) / 2 +
                            (i % 2 == 0) * (Groove->GetEntry(i / 2) % 2 == 1));
...
void CGrooveDlg::OnBnClickedButtonGrooveGenerate() {
  ...
  int Denom = atoi(Str);
  if (Denom < 1 || Denom > MAX_GROOVE_SIZE || Num < Denom || Num > Denom * 255)
    return;
