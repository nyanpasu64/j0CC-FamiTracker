...
CFrameClipData::CFrameClipData() { memset(&ClipInfo, 0, sizeof(ClipInfo)); }
...
void CFrameSelection::Normalize(CFrameCursorPos &Begin,
                                CFrameCursorPos &End) const {
...
CFrameIterator::CFrameIterator(const CFrameIterator &it)
    : m_iTrack(it.m_iTrack), m_pDocument(it.m_pDocument),
      CFrameCursorPos(static_cast<const CFrameCursorPos &>(it)) {}

CFrameIterator::CFrameIterator(CFamiTrackerDoc *pDoc, int Track,
...
CFrameIterator::FromCursor(const CFrameCursorPos &Pos,
                           CFamiTrackerDoc *const pDoc, int Track) {
  return std::make_pair(CFrameIterator{pDoc, Track, Pos},
                        CFrameIterator{pDoc, Track, Pos});
}

std::pair<CFrameIterator, CFrameIterator>
CFrameIterator::FromSelection(const CFrameSelection &Sel,
                              CFamiTrackerDoc *const pDoc, int Track) {
  CFrameCursorPos it, end;
  Sel.Normalize(it, end);
  return std::make_pair(CFrameIterator{pDoc, Track, it},
                        CFrameIterator{pDoc, Track, end});
}

int CFrameIterator::Get(int Channel) const {
  return m_pDocument->GetPatternAtFrame(m_iTrack, NormalizeFrame(m_iFrame),
                                        Channel);
}

void CFrameIterator::Set(int Channel, int Frame) {
  m_pDocument->SetPatternAtFrame(m_iTrack, NormalizeFrame(m_iFrame), Channel,
                                 Frame);
}
...
CFrameIterator &CFrameIterator::operator++() { return operator+=(1); }
