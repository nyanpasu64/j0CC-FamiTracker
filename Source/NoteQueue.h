...
/*!
        \brief A queue which automatically reassigns notes in the same logical
   track to different physical channels. The same note coming from the same
   channel in a track may only be played on one physical channel.
*/
...
public:
  ...
  /*!	\brief Stops whatever is played from a specific channel.
          \param Channel The channel index.
          \return A vector containing the physical channel indices that have a
     note halted. */
...
/*!
        \brief The actual note queue that keeps track of multiple logical
   tracks.
*/
class CNoteQueue {
public:
