...
bool CSequence::operator==(const CSequence &other) // // //
{
  return m_iItemCount == other.m_iItemCount &&
         ...
         memcmp(m_cValues, other.m_cValues,
                m_iItemCount * sizeof(*m_cValues)) == 0;
}
...
signed char CSequence::GetItem(int Index) const { return m_cValues[Index]; }

unsigned int CSequence::GetItemCount() const { return m_iItemCount; }

unsigned int CSequence::GetLoopPoint() const { return m_iLoopPoint; }

unsigned int CSequence::GetReleasePoint() const { return m_iReleasePoint; }
