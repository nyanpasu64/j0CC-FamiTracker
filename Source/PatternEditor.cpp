...
void CPatternEditor::ApplyColorScheme() {
  ...
  const int fontSize =
      calculateFontSize(rowHeight, settings->Appearance.fontPercent);
...
void CPatternEditor::DrawCell(CDC *pDC, int PosX, cursor_column_t Column,
  ...
  effect_t EffNumber =
      Column >= 4 ? pNoteData->EffNumber[(Column - 4) / 3] : EF_NONE; // // //
  ...
  auto BAR = [&](int x) {
    pDC->FillSolidRect(x + halfX - BAR_WIDTH / 2, halfY - BAR_HEIGHT / 2,
                       BAR_WIDTH, BAR_HEIGHT, pColorInfo->Shaded);
  };
          ...
          for (unsigned int i = 0;
            ...
            if (pNoteData->EffNumber[i] != EF_NONE) {
              DrawChar(pDC, PosX + m_iCharWidth / 2, PosY,
                       EFF_CHAR[pNoteData->EffNumber[i]], DimEff);
              DrawChar(pDC, PosX + m_iCharWidth * 3 / 2, PosY,
  ...
  case C_EFF4_NUM:
    ...
    if (EffNumber == 0)
      ...
      DrawChar(pDC, PosX + m_iCharWidth / 2, PosY, EFF_CHAR[EffNumber],
               EffColor); // // //
    break;
  case C_EFF1_PARAM1:
  case C_EFF2_PARAM1:
...
void CPatternEditor::DrawRegisters(CDC *pDC) {
  ...
  for (int i = 0; i < 5; ++i) {
    GetRegsFunc(
        SNDCHIP_NONE, [&](int x) { return 0x4000 + i * 4 + x; }, 4);
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_VRC6)) {
    ...
    for (int i = 0; i < 3; ++i) {
      GetRegsFunc(
          SNDCHIP_VRC6, [&](int x) { return 0x9000 + i * 0x1000 + x; }, 3);
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_MMC5)) { // // //
    ...
    for (int i = 0; i < 2; ++i) {
      GetRegsFunc(
          SNDCHIP_MMC5, [&](int x) { return 0x5000 + i * 4 + x; }, 4);
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_N163)) {
    ...
    for (int i = 0; i < 16; ++i) {
      GetRegsFunc(
          SNDCHIP_N163, [&](int x) { return i * 8 + x; }, 8);
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_FDS)) {
    ...
    for (int i = 0; i < 11; ++i) {
      GetRegsFunc(
          SNDCHIP_FDS, [&](int) { return 0x4080 + i; }, 1);
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_VRC7)) { // // //
    ...
    GetRegsFunc(
        SNDCHIP_VRC7, [](int x) { return x; }, 8);
    DrawRegFunc(_T("$00:"), 8); // // //

    for (int i = 0; i < 6; ++i) {
      GetRegsFunc(
          SNDCHIP_VRC7, [&](int x) { return i + (++x << 4); }, 3);
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_S5B)) { // // //
    ...
    for (int i = 0; i < 4; ++i) {
      GetRegsFunc(
          SNDCHIP_S5B, [&](int x) { return i * 2 + x; }, 2);
    ...
    for (int i = 0; i < 2; ++i) {
      GetRegsFunc(
          SNDCHIP_S5B, [&](int x) { return i * 3 + x + 8; }, 3);
