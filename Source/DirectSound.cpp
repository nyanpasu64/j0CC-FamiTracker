...
BOOL CALLBACK CDSound::DSEnumCallback(LPGUID lpGuid, LPCTSTR lpcstrDescription,
                                      LPCTSTR lpcstrModule, LPVOID lpContext) {
  return pThisObject->EnumerateCallback(lpGuid, lpcstrDescription, lpcstrModule,
                                        lpContext);
}
...
CDSound::CDSound(HWND hWnd, HANDLE hNotification)
    : m_iDevices(0), m_lpDirectSound(NULL), m_hWndTarget(hWnd),
...
bool CDSound::SetupDevice(int iDevice) {
  ...
  if (FAILED(DirectSoundCreate((LPCGUID)m_pGUIDs[iDevice], &m_lpDirectSound,
                               NULL))) {
    m_lpDirectSound = NULL;
    return false;
  }
  ...
  if (FAILED(
          m_lpDirectSound->SetCooperativeLevel(m_hWndTarget, DSSCL_PRIORITY))) {
    m_lpDirectSound = NULL;
    return false;
  }
...
CDSoundChannel *CDSound::OpenChannel(int SampleRate, int SampleSize,
  ...
  if (FAILED(m_lpDirectSound->CreateSoundBuffer(
          &dsbd, &pChannel->m_lpDirectSoundBuffer, NULL))) {
    delete pChannel;
    return NULL;
  }
  ...
  if (FAILED(pChannel->m_lpDirectSoundBuffer->QueryInterface(
          IID_IDirectSoundNotify, (void **)&pChannel->m_lpDirectSoundNotify))) {
    delete pChannel;
    return NULL;
  }

  if (FAILED(pChannel->m_lpDirectSoundNotify->SetNotificationPositions(Blocks,
                                                                       dspn))) {
    delete pChannel;
    return NULL;
  }
...
bool CDSoundChannel::Play() const {
  ...
  return FAILED(m_lpDirectSoundBuffer->Play(NULL, NULL, DSBPLAY_LOOPING))
             ? false
             : true;
}
...
bool CDSoundChannel::ClearBuffer() {
  ...
  if (FAILED(m_lpDirectSoundBuffer->Lock(
          0, m_iSoundBufferSize, (void **)&pAudioPtr1, &AudioBytes1,
          (void **)&pAudioPtr2, &AudioBytes2, 0)))
    return false;
  ...
  if (FAILED(m_lpDirectSoundBuffer->Unlock((void *)pAudioPtr1, AudioBytes1,
                                           (void *)pAudioPtr2, AudioBytes2)))
    return false;
...
bool CDSoundChannel::WriteBuffer(char const *pBuffer, unsigned int Samples) {
  ...
  if (FAILED(m_lpDirectSoundBuffer->Lock(
          Block * m_iBlockSize, m_iBlockSize, (void **)&pAudioPtr1,
          &AudioBytes1, (void **)&pAudioPtr2, &AudioBytes2, 0)))
    return false;
...
buffer_event_t CDSoundChannel::WaitForSyncEvent(DWORD dwTimeout) const {
  ...
  case WAIT_OBJECT_0 + 1: // DirectSound buffer
    return (GetWritableBlock() == m_iCurrentWriteBlock) ? BUFFER_OUT_OF_SYNC
                                                        : BUFFER_IN_SYNC;
