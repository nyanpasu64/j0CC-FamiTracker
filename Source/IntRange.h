...
/*!
        \brief Simple class for integer range generator. A single iteration
   object is stateless, so every use in a range-for loop always iterates through
   all values.
*/
template <class T> struct CIntRange {
private:
  struct it;
...
public:
  ...
  constexpr T operator[](size_t x) const { return _b + x; }
  template <class _T>
  friend constexpr bool operator==(const CIntRange<_T> &lhs,
                                   const CIntRange<_T> &rhs) {
  ...
  friend /*constexpr*/ CIntRange<_T> operator&(const CIntRange<_T> &lhs,
                                               const CIntRange<_T> &rhs) {
  ...
  template <class _T> // shift
  friend /*constexpr*/ CIntRange<_T> operator+(T lhs,
                                               const CIntRange<_T> &rhs) {
    return rhs + lhs;
  }
  template <class _T> // shift
  ...
  CIntRange operator++(int) { return CIntRange<T>(_b++, _e++); }
  CIntRange operator--(int) { return CIntRange<T>(_b--, _e--); }
...
constexpr CIntRange<std::common_type_t<T, U>> make_int_range(T b, U e) {
