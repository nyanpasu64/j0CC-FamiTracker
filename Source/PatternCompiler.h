...
public:
  CPatternCompiler(CFamiTrackerDoc *pDoc, unsigned int *pInstList,
                   DPCM_List_t *pDPCMList, CCompilerLog *pLogger);
  ~CPatternCompiler();
...
private:
  ...
  void ScanNoteLengths(stSpacingInfo &Info, int Track, unsigned int StartRow,
                       int Pattern, int Channel);
...
private:
  ...
  bool
      m_bDSamplesAccessed[OCTAVE_RANGE * NOTE_RANGE]; // <- check the range, its
                                                      // not optimal right now
