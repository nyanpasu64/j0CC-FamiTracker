...
/*!
        \brief A class that manages a dynamic list of bookmarks for a single
   track.
*/
...
public:
  ...
  */
  ...
  /*!	\brief Obtains a bookmark at a given position.
          \details The bookmark is owned by the collection. The bookmark shall
     not be deallocated via the returned pointer. \param Index Position index.
  ...
  */
  ...
  /*!	\brief Replaces a bookmark to the collection at a given position.
          ...
          \param pMark Pointer to the bookmark.
          \return Whether the bookmark is different from the original one at the
     given position.
  */
  ...
  */
  ...
  /*!	\brief Inserts frames into the current track, shifting the positions of
     all bookmarks below. \details This method does not perform validation on
     the frame values. \param Frame The frame index. \param Count Number of
     frames to insert.
  */
  void InsertFrames(unsigned Frame, unsigned Count);
  /*!	\brief Removes frames into the current track, deleting bookmarks in
     these frames and shifting the positions of all bookmarks below. \details
     This method does not perform validation on the frame values. \param Frame
     The frame index. \param Count Number of frames to remove.
  */
  void RemoveFrames(unsigned Frame, unsigned Count);
  /*!	\brief Exchanges the frame values of all bookmarks in either of the
     given frames. \param A Frame index. \param B Frame index.
  */
  void SwapFrames(unsigned A, unsigned B);

  /*!	\brief Locates a bookmark in the collection.
          \param pMark Pointer to the bookmark.
          \return The bookmark index, or -1 if the bookmark is not contained by
     the collection.
  */
  int GetBookmarkIndex(const CBookmark *const pMark) const;
  /*!	\brief Locates the first bookmark below a given position.
          \details This method returns the bookmark with the least index if
     multiple bookmarks for the same location exist. It always returns a
     bookmark whose position is not equal to the given values whenever possible.
          \param Frame The frame index.
          \param Row The row index.
          \return Pointer to the first bookmark, or nullptr if no bookmarks
     exist.
  */
  ...
  */
  ...
  /*!	\brief Locates the bookmark at a given position.
          \details This method returns the bookmark with the least index if
     multiple bookmarks for the same location exist. \param Frame The frame
     index. \param Row The row index. \return Pointer to the bookmark, or
     nullptr if no bookmarks exist at the position.
  */
  ...
  */
  ...
  /*!	\brief Sorts the contained bookmarks by their names.
          \details The method performs a stable sort; relative positions of
     equal bookmarks remain unchanged after calling this method. \param Desc
     Whether bookmarks are sorted descending (true) or ascending (false).
          \return Whether the position of any bookmark is changed.
  */
