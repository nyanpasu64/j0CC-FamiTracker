...
CDetuneTable::CDetuneTable(type_t Type, int Low, int High)
    : m_iType(Type), m_iRangeLow(Low), m_iRangeHigh(High),
      m_iRegisterValue(96, 0U), m_iOffsetValue(96, 0U) {}

CDetuneTable::type_t CDetuneTable::GetType() const { return m_iType; }

size_t CDetuneTable::GetNoteCount() const { return m_iRegisterValue.size(); }
...
void CDetuneTable::SetGenerator(GenFunc f) { m_fFunction = f; }
...
void CDetuneTable::GenerateRegisters(double LowestNote) {
  ...
  for (size_t i = 0; i < Count; ++i)
    m_iRegisterValue[i] =
        static_cast<unsigned>(GetDefaultReg(LowestNote++) + .5);
}
...
CDetuneN163::CDetuneN163()
    : CDetuneTable(DETUNE_N163, 0, 0xFFFF), m_iChannelCount(1) {
  SetGenerator([&](double x) {
    return x / BASE_FREQ_NTSC * 15. * (1 << 18) * m_iChannelCount;
  });
  SetFrequencyFunc([&](double x) {
    return BASE_FREQ_NTSC * x / 15. / (1 << 18) / m_iChannelCount;
  });
  GenerateRegisters();
}
...
CDetuneS5B::CDetuneS5B() : CDetuneTable(DETUNE_S5B, 0, 0xFFF) {
