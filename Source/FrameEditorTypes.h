...
struct CFrameCursorPos {
  ...
  CFrameCursorPos(int Frame, int Channel)
      : m_iFrame(Frame), m_iChannel(Channel) {}
...
public:
  ...
  CFrameIterator(CFamiTrackerDoc *const pDoc, int Track,
                 const CFrameCursorPos &Pos);
  CFrameIterator(const CFamiTrackerDoc *const pDoc, int Track,
                 const CFrameCursorPos &Pos);

  static std::pair<CFrameIterator, CFrameIterator>
  FromCursor(const CFrameCursorPos &Pos, CFamiTrackerDoc *const pDoc,
             int Track);
  static std::pair<CFrameIterator, CFrameIterator>
  FromSelection(const CFrameSelection &Sel, CFamiTrackerDoc *const pDoc,
                int Track);
