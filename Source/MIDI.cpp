...
bool CMIDI::OpenDevices(void) {
  ...
  if (m_iInDevice != 0) {
    ...
    Result = midiInOpen(&m_hMIDIIn, m_iInDevice - 1, (DWORD_PTR)MidiInProc, 0,
                        CALLBACK_FUNCTION);
  ...
  if (m_iOutDevice != 0) {
    ...
    // Set patches
    midiOutShortMsg(m_hMIDIOut,
                    (MIDI_MSG_PROGRAM_CHANGE << 4 | 0x00) | (1 << 8));
    midiOutShortMsg(m_hMIDIOut,
                    (MIDI_MSG_PROGRAM_CHANGE << 4 | 0x01) | (1 << 8));
    midiOutShortMsg(m_hMIDIOut,
                    (MIDI_MSG_PROGRAM_CHANGE << 4 | 0x02) | (74 << 8));
    midiOutShortMsg(m_hMIDIOut,
                    (MIDI_MSG_PROGRAM_CHANGE << 4 | 0x03) | (115 << 8));
    midiOutShortMsg(m_hMIDIOut,
                    (MIDI_MSG_PROGRAM_CHANGE << 4 | 0x04) | (118 << 8));
...
int CMIDI::GetNumInputDevices() const { return midiInGetNumDevs(); }

int CMIDI::GetNumOutputDevices() const { return midiOutGetNumDevs(); }
...
void CMIDI::ResetOutput() { midiOutReset(m_hMIDIOut); }
