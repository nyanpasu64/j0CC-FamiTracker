...
CSeqInstHandlerN163::CSeqInstHandlerN163(CChannelHandlerInterface *pInterface,
                                         int Vol, int Duty)
    : CSeqInstHandler(pInterface, Vol, Duty), m_cBuffer(),
      m_pBufferCurrent(m_cBuffer), m_bForceUpdate(false),
      m_pBufferPrevious(m_cBuffer + CInstrumentN163::MAX_WAVE_SIZE) {}

void CSeqInstHandlerN163::LoadInstrument(
    std::shared_ptr<CInstrument> pInst) // // //
{
  CSeqInstHandler::LoadInstrument(pInst);
  CChannelHandlerInterfaceN163 *pInterface =
      dynamic_cast<CChannelHandlerInterfaceN163 *>(m_pInterface);
  if (pInterface == nullptr)
    return;
  auto pN163Inst =
      std::dynamic_pointer_cast<const CInstrumentN163>(m_pInstrument);
  if (pN163Inst == nullptr)
    return;
...
void CSeqInstHandlerN163::RequestWaveUpdate() { m_bForceUpdate = true; }
...
void CSeqInstHandlerN163::UpdateWave(const CInstrumentN163 *pInst) {
  ...
  for (int i = 0; i < Count; ++i)
    m_pBufferCurrent[i] = pInst->GetSample(Index, 2 * i) |
                          (pInst->GetSample(Index, 2 * i + 1) << 4);

  if (auto pInterface =
          dynamic_cast<CChannelHandlerInterfaceN163 *>(m_pInterface))
    if (memcmp(m_pBufferCurrent, m_pBufferPrevious, Count) != 0 ||
        m_bForceUpdate)
