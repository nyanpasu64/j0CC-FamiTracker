...
CDSample::CDSample(unsigned int Size)
    : m_iSampleSize(Size), m_pSampleData(new char[Size]),
...
#pragma warning(disable : 4717) // "recursive on all control paths, function
...
unsigned int CDSample::GetSize() const { return m_iSampleSize; }

char *CDSample::GetData() const { return m_pSampleData.get(); }

void CDSample::SetName(const char *pName) {
  strncpy_s(m_pName.get(), MAX_NAME_SIZE, pName, MAX_NAME_SIZE);
}

const char *CDSample::GetName() const { return m_pName.get(); }
