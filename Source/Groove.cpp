...
CGroove::CGroove(int Speed) { Clear(Speed); }
...
unsigned char CGroove::GetSize() const { return m_iLength; }

void CGroove::SetSize(unsigned char Size) {
  if (Size > MAX_GROOVE_SIZE)
    return;
  if (m_iLength < Size)
    for (unsigned char i = m_iLength; i < Size; i++)
      m_iEntry[i] = 0;
  m_iLength = Size;
}
...
float CGroove::GetAverage() const {
  float Total = 0;
  if (!m_iLength)
    return 6.0; // return DEFAULT_SPEED;
  else {
    for (unsigned char i = 0; i < m_iLength; i++)
      Total += m_iEntry[i];
