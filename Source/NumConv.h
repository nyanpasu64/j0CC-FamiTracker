...
template <typename T>
constexpr std::optional<T> to_unsigned(std::string_view sv,
...
constexpr std::optional<T> to_signed(std::string_view sv,
                                     unsigned radix) noexcept {
  bool negative = false;
  if (!sv.empty()) {
    if (sv.front() == '-') {
...
template <typename T> constexpr char to_digit(T x) noexcept {
  if (x >= T(0)) {
    if (x < T(10))
...
template <unsigned Radix> struct str_buf_ {
...
char *from_uint_impl(char (&data)[BufLen], uintmax_t x,
                     unsigned places) noexcept {
...
template <unsigned Radix, unsigned BufLen>
std::string_view from_uint(char (&data)[BufLen], uintmax_t x,
                           unsigned places) noexcept {
...
#endif
template <unsigned Radix, unsigned BufLen>
std::string_view from_int(char (&data)[BufLen], intmax_t x,
                          unsigned places) noexcept {
  if (x >= 0)
    return from_uint_impl<Radix>(data, x, places);
...
inline std::string_view sv_from_uint(uintmax_t x,
                                     unsigned places = (unsigned)-1) noexcept {
...
sv_from_uint_bin(uintmax_t x, unsigned places = (unsigned)-1) noexcept {
...
// every sv_* function shares the same buffer in each thread
inline std::string_view
sv_from_uint_oct(uintmax_t x, unsigned places = (unsigned)-1) noexcept {
...
// every sv_* function shares the same buffer in each thread
inline std::string_view
sv_from_uint_hex(uintmax_t x, unsigned places = (unsigned)-1) noexcept {
  return details::from_uint<16>(details::str_buf_<16>::data, x, places);
}

// converts a signed integer to a decimal string
// every sv_* function shares the same buffer in each thread
inline std::string_view sv_from_int(intmax_t x,
                                    unsigned places = (unsigned)-1) noexcept {
...
// every sv_* function shares the same buffer in each thread
inline std::string_view
sv_from_int_bin(intmax_t x, unsigned places = (unsigned)-1) noexcept {
...
// every sv_* function shares the same buffer in each thread
inline std::string_view
sv_from_int_oct(intmax_t x, unsigned places = (unsigned)-1) noexcept {
...
// every sv_* function shares the same buffer in each thread
inline std::string_view
sv_from_int_hex(intmax_t x, unsigned places = (unsigned)-1) noexcept {
...
inline std::string time_from_int(intmax_t seconds) {
  return seconds >= 0 ? time_from_uint(seconds)
                      : ('-' + time_from_uint(-seconds));
}
