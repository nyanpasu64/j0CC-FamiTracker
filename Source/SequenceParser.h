...
/*!
        \brief A realization of the CSequenceStringConversion class from
   FamiTracker 0.5.0 beta, which handles conversion between sequence values and
   MML string terms.
*/
...
public:
  ...
  virtual std::string
  ToString(char Value) const = 0; // TODO: maybe use a different class for this

  /*!	\brief Converts an MML string term to a sequence value.
          \details A string term may represent any number of sequence values,
     including zero. \param String A single MML string term.
  ...
  /*!	\brief Checks the availability of the current string conversion.
          \details For each valid MML string term, **as well as before and after
     converting an MML string**, a new value is expected as long as this method
     returns true. \return True if a new value is ready for use. */
...
public:
  ...
  CSeqConversionDefault(int Min, int Max)
      : m_iMinValue(Min), m_iMaxValue(Max) {}
...
protected:
  bool GetNextInteger(std::string::const_iterator &b,
                      std::string::const_iterator &e, int &Out,
                      bool Signed = false) const;
  virtual bool GetNextTerm(std::string::const_iterator &b,
                           std::string::const_iterator &e, int &Out);
...
protected:
  bool GetNextTerm(std::string::const_iterator &b,
                   std::string::const_iterator &e, int &Out) override;
...
/*!
        \brief A class which uses a conversion object to translate between
   string representations and instrument sequence values.
*/
...
public:
  ...
  /*!	\brief Changes the conversion algorithm.
          \details The sequence parser owns the conversion object and handles
     its deletion. \param pConv Pointer to the new sequence conversion object.
   */
  ...
  /*!	\brief Obtains a string representation of the current instrument
     sequence. \return An MML string which represents the sequence. */
