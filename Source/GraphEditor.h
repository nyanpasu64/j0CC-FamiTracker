...
enum edit_t { EDIT_NONE, EDIT_LINE, EDIT_POINT, EDIT_LOOP, EDIT_RELEASE };
...
protected:
  ...
  void DrawBackground(CDC *pDC, int Lines, bool DrawMarks,
...
public:
  CBarGraphEditor(CSequence *pSequence, int Levels)
      : CGraphEditor(pSequence), m_iLevels(Levels){};
...
public:
  CNoiseEditor(CSequence *pSequence, int Items)
      : CGraphEditor(pSequence), m_iItems(Items), m_iLastIndex(-1){};
  afx_msg void OnPaint();
  void ModifyItem(CPoint point, bool Redraw);
  void HighlightItem(CPoint point);
  int GetItemHeight() const;
