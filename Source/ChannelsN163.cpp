...
CChannelHandlerN163::CChannelHandlerN163()
    : FrequencyChannelHandler(0xFFFF, 0x0F), m_bDisableLoad(false), // // //
...
bool CChannelHandlerN163::HandleEffect(effect_t EffNum,
  ...
  case EF_N163_WAVE_BUFFER: // // //
    ...
    break;
  default:
    return FrequencyChannelHandler::HandleEffect(EffNum, EffParam);
  }
...
CString CChannelHandlerN163::GetSlideEffectString() const // // //
{
  ...
  case EF_PORTA_UP:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_DOWN],
                       m_iPortaSpeed >> N163_PITCH_SLIDE_SHIFT);
    break;
  case EF_PORTA_DOWN:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_UP],
                       m_iPortaSpeed >> N163_PITCH_SLIDE_SHIFT);
    break;
  case EF_PORTAMENTO:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect],
                       m_iPortaSpeed >> N163_PITCH_SLIDE_SHIFT);
    break;
  }
