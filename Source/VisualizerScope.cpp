...
CVisualizerScope::CVisualizerScope(bool bBlur)
    : m_pWindowBuf(NULL), m_bBlur(bBlur), m_iWindowBufPtr(0) {}

CVisualizerScope::~CVisualizerScope() { SAFE_RELEASE_ARRAY(m_pWindowBuf); }
...
void CVisualizerScope::ClearBackground() {
  for (int y = 0; y < m_iHeight; ++y) {
    int intensity = static_cast<int>(
        sinf((float(y) * 3.14f) / float(m_iHeight)) * 40.0f); // // //
    memset(&m_pBlitBuffer[y * m_iWidth], intensity,
...
void CVisualizerScope::RenderBuffer() {
  ...
  for (float x = 0.0f; x < float(m_iWidth); ++x) {
    ...
    PutPixel(m_pBlitBuffer.get(), m_iWidth, m_iHeight, x,
             Sample + HALF_HEIGHT - 0.5f, LINE_COL2);
    PutPixel(m_pBlitBuffer.get(), m_iWidth, m_iHeight, x,
             Sample + HALF_HEIGHT + 0.5f, LINE_COL2);
    PutPixel(m_pBlitBuffer.get(), m_iWidth, m_iHeight, x,
             Sample + HALF_HEIGHT + 0.0f, LINE_COL1);
    ...
    if ((Sample - LastSample) > 1.0f) {
      ...
      for (float y = LastSample; y < Sample; ++y) {
        ...
        PutPixel(m_pBlitBuffer.get(), m_iWidth, m_iHeight, x + Offset - 1.0f,
                 y + HALF_HEIGHT + frac, LINE_COL1);
      }
      ...
      for (float y = Sample; y < LastSample; ++y) {
        ...
        PutPixel(m_pBlitBuffer.get(), m_iWidth, m_iHeight, x - Offset,
                 y + HALF_HEIGHT + frac, LINE_COL1);
      }
    }
  }
