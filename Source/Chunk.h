...
public:
  CChunkDataReference(CStringA refName)
      : CChunkData(), m_refName(refName), ref(-1) {}
  int GetSize() const { return 2; }
...
public:
  CChunkDataBank(CStringA bankOf, int bank)
      : CChunkData(), m_bankOf(bankOf), m_bank(bank) {}
  int GetSize() const { return 1; }
  unsigned short GetData() const { return m_bank; };
  CStringA m_bankOf; // Reference to a label which belongs to the bank this data
                     // should point to
  unsigned int m_bank;
};
...
public:
  CChunkDataString(const std::vector<char> &data)
      : CChunkData(), m_vData(data) {}
