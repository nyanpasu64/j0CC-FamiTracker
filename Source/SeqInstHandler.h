...
/*!
        ...
        \details Sequence instruments are the major type of instruments provided
   by FamiTracker; they are used by all sound channels except for the Konami
   VRC7.
*/
...
public:
  ...
  enum seq_state_t {    // // //
    ...
    SEQ_STATE_END,      /*!< Current sequence has just finished running the last
                           tick. */
    SEQ_STATE_HALT /*!< Current sequence has finished running until the next
                      note. */
  };
  ...
  /*!	\brief Obtains the current sequence state of a given sequence type.
          \param Index The sequence type, which should be a member of
     sequence_t. \return The sequence state of the given sequence type. */
...
protected:
  ...
  /*!	\brief Prepares a sequence type for use by
     CSeqInstHandler::UpdateInstrument. \param Index The sequence type. \param
     pSequence Pointer to the sequence. */
...
protected:
  /*!	\brief An array holding pointers to the sequences used by the current
   * instrument. */
  const CSequence *m_pSequence[SEQ_COUNT];
  /*!	\brief An array holding the states of each sequence type used in
   * sequence instruments. */
  seq_state_t m_iSeqState[SEQ_COUNT];
  /*!	\brief An array holding the tick index of each sequence type used in
   * sequence instruments. */
  int m_iSeqPointer[SEQ_COUNT];
  /*!	\brief The current duty cycle of the instrument.
          \details The exact interpretation of this member may not be identical
     across sound channels. \warning Currently unused. */
  int m_iDutyParam;
  /*!	\brief The default duty cycle of the instrument.
          \details On triggering a new note, the duty cycle value is reset to
