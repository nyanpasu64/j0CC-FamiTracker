...
static void __GradientRectTriple(CDC *pDC, int x, int y, int r, int b,
...
static void __GradientBar(CDC *pDC, int x, int y, int r, int b, COLORREF col_fg,
                          COLORREF col_bg) {
...
static void __GradientRect(CDC *pDC, int x, int y, int r, int b,
                           COLORREF top_col, COLORREF bottom_col) {
  TRIVERTEX Vertices[2];
  GRADIENT_RECT Rect;
...
void GradientBar(CDC *pDC, const CRect &r, COLORREF col_fg,
                 COLORREF col_bg) // // //
{
...
void BlurBuffer(COLORREF *pBuffer, int Width, int Height,
  ...
  for (int x = 1; x < Width - 1; ++x) {
    for (int y = 1; y < Height - 1; ++y) {
      ...
      int r = (RED(Col1) + RED(Col2) + RED(Col3) + RED(Col4) + RED(Col5) +
               RED(Col6) + RED(Col7) + RED(Col8)) >>
              3;
      int g = (GREEN(Col1) + GREEN(Col2) + GREEN(Col3) + GREEN(Col4) +
               GREEN(Col5) + GREEN(Col6) + GREEN(Col7) + GREEN(Col8)) >>
              3;
      int b = (BLUE(Col1) + BLUE(Col2) + BLUE(Col3) + BLUE(Col4) + BLUE(Col5) +
               BLUE(Col6) + BLUE(Col7) + BLUE(Col8)) >>
              3;
...
void PutPixel(COLORREF *pBuffer, int Width, int Height, float x, float y,
  ...
  COLORREF c1 =
      BLEND(col, pBuffer[y0 * Width + x0], int((w_x0 * w_y0) * 100.0f));
  COLORREF c2 =
      BLEND(col, pBuffer[y0 * Width + x1], int((w_x1 * w_y0) * 100.0f));
  COLORREF c3 =
      BLEND(col, pBuffer[y1 * Width + x0], int((w_x0 * w_y1) * 100.0f));
  COLORREF c4 =
      BLEND(col, pBuffer[y1 * Width + x1], int((w_x1 * w_y1) * 100.0f));
