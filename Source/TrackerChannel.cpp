...
bool CTrackerChannel::IsEffectCompatible(effect_t EffNumber,
                                         int EffParam) const {
  switch (EffNumber) {
  case EF_NONE:
  case EF_SPEED:
  ...
  case EF_DPCM_PITCH: {
    ...
    case EF_RETRIGGER:
       ...
       * Had mRetriggerCtr been an u8, XFF would assign mRetriggerCtr=0 and
       * trigger DPCM.
  ...
  case EF_DUTY_CYCLE: {
    ...
    // Don't use make_unique if you need a custom deleter or are adopting a raw
    // pointer from elsewhere.

    auto channelHandler =
        std::unique_ptr<CChannelHandler>(F.Produce(this->m_iChannelID));
  ...
  default:
    throw std::runtime_error(
        "Missing case in CTrackerChannel::IsEffectCompatible");
  }
