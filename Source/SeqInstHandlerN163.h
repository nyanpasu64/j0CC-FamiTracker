...
public:
  /*!	\brief Constructor of the N163 sequence instrument handler.
          \warning As of now, the actual loading of instrument waveforms takes
     place in the channel handler rather than through its interface. \param
     pInterface Pointer to the channel interface. \param Vol Default volume for
     instruments used by this handler. \param Duty Default duty cycle for
     instruments used by this handler. */
  ...
  /*!	\brief Loads a new instrument into the instrument handler.
          \details This reimplementation sets up the wave size, wave position,
     and wave count of the channel handler. \param pInst Pointer to the
  ...
  /*!	\brief Requests the instrument handler to overwrite the wave buffer for
   * the next tick. */
