...
int CSequenceManager::GetCount() const { return m_pCollection.size(); }

CSequenceCollection *CSequenceManager::GetCollection(unsigned int Index) {
  if (Index >= m_pCollection.size())
    return nullptr;
...
const CSequenceCollection *
CSequenceManager::GetCollection(unsigned int Index) const {
  if (Index >= m_pCollection.size())
    return nullptr;
  return m_pCollection[Index].get();
}
