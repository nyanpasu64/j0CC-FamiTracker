...
constexpr std::array<T, N> make_hann_window() {
  ...
  for (int i = 0; i < N; ++i)
    window[i] = [](T x) {
      return x * x;
    }(FFT::details::remez_sin(-i * fraction));
  return window;
}

} // namespace details

template <std::size_t N> class FftBuffer {
public:
  static constexpr std::size_t GetPoints() noexcept { return N; }
  ...
  void Transform() { FFT::transform_fwd(samples_, buffer_, window.cbegin()); }
  ...
  void CopyIn(InputIt Samples, std::size_t SampleCount) {
    ...
    std::copy(samples_.cbegin() + SampleCount, samples_.cend(),
              samples_.begin());
    std::transform(Samples, Samples + SampleCount, samples_.begin(),
                   [](auto x) { return std::complex<double>(x, 0); });
  }
