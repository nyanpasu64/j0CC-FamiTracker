...
bool CSeqConversionDefault::IsReady() const { return m_bReady; }
...
void CSeqConversionDefault::OnStart() { m_bHex = false; }

void CSeqConversionDefault::OnFinish() { m_bReady = false; }

bool CSeqConversionDefault::GetNextInteger(std::string::const_iterator &b,
                                           std::string::const_iterator &e,
                                           int &Out, bool Signed) const {
  std::smatch m;
  ...
  try {
    if (m_bHex) {
      ...
      static const std::regex HEX_PREFIX_RE{
          R"(([\+-]?)[\$x]([0-9A-Fa-f]+))"}; // do not allow 0x prefix
...
bool CSeqConversionDefault::GetNextTerm(std::string::const_iterator &b,
                                        std::string::const_iterator &e,
                                        int &Out) {
...
bool CSeqConversion5B::GetNextTerm(std::string::const_iterator &b,
...
std::string CSeqConversionArpScheme::ToString(char Value) const // // //
{
  ...
  if (!Offset) {
    switch (Scheme) {
    case ARPSCHEME_MODE_X:
      return "x";
    case ARPSCHEME_MODE_Y:
      return "y";
    case ARPSCHEME_MODE_NEG_Y:
      return "-y";
    default:
      return "0";
    }
...
bool CSeqConversionArpScheme::GetNextTerm(std::string::const_iterator &b,
                                          std::string::const_iterator &e,
                                          int &Out) {
...
bool CSeqConversionArpFixed::GetNextTerm(std::string::const_iterator &b,
                                         std::string::const_iterator &e,
                                         int &Out) {
  auto _b(b), _e(e);
...
void CSequenceParser::ParseSequence(const std::string &String) {
  ...
  const auto PushFunc = [&]() {
    for (; m_pConversion->IsReady() && m_iPushedCount < MAX_SEQUENCE_ITEMS;
         ++m_iPushedCount) {
