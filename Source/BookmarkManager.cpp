...
CBookmarkManager::CBookmarkManager(unsigned Count) : m_pCollection(Count) {
  ClearAll();
}
...
void CBookmarkManager::SetCollection(unsigned Track,
                                     CBookmarkCollection *const pCol) {
...
void CBookmarkManager::InsertTrack(unsigned Track) {
  ...
  m_pCollection.insert(
      m_pCollection.begin() + Track,
      std::unique_ptr<CBookmarkCollection>(new CBookmarkCollection()));
}
