...
CBookmark *CBookmarkDlg::MakeBookmark() const {
  ...
  CBookmark *pMark =
      new CBookmark(m_cSpinFrame->GetPos(), m_cSpinRow->GetPos());
  pMark->m_Highlight.First =
      m_bEnableHighlight1 ? m_cSpinHighlight1->GetPos() : -1;
  pMark->m_Highlight.Second =
      m_bEnableHighlight2 ? m_cSpinHighlight2->GetPos() : -1;
...
void CBookmarkDlg::OnOK() { OnBnClickedButtonBookmarkUpdate(); }
...
void CBookmarkDlg::OnBnClickedButtonBookmarkSortp() {
  if (m_pCollection->GetCount()) {
    const CBookmark *pMark =
        m_pCollection->GetBookmark(m_cListBookmark->GetCurSel());
...
void CBookmarkDlg::OnLbnSelchangeListBookmarks() {
  ...
  static_cast<CButton *>(GetDlgItem(IDC_CHECK_BOOKMARK_PERSIST))
      ->SetCheck(m_bPersist ? BST_CHECKED : BST_UNCHECKED);
  GetDlgItem(IDC_EDIT_BOOKMARK_NAME)->SetWindowText(pMark->m_sName.c_str());

  m_bEnableHighlight1 = pMark->m_Highlight.First != -1;
  m_cSpinHighlight1->SetPos(m_bEnableHighlight1
                                ? pMark->m_Highlight.First
                                : m_pDocument->GetHighlight().First);
  static_cast<CButton *>(GetDlgItem(IDC_CHECK_BOOKMARK_HIGH1))
      ->SetCheck(m_bEnableHighlight1);
  static_cast<CEdit *>(GetDlgItem(IDC_EDIT_BOOKMARK_HIGH1))
      ->EnableWindow(m_bEnableHighlight1);

  m_bEnableHighlight2 = pMark->m_Highlight.Second != -1;
  m_cSpinHighlight2->SetPos(m_bEnableHighlight2
                                ? pMark->m_Highlight.Second
                                : m_pDocument->GetHighlight().Second);
  static_cast<CButton *>(GetDlgItem(IDC_CHECK_BOOKMARK_HIGH2))
      ->SetCheck(m_bEnableHighlight2);
  static_cast<CEdit *>(GetDlgItem(IDC_EDIT_BOOKMARK_HIGH2))
      ->EnableWindow(m_bEnableHighlight2);
...
void CBookmarkDlg::OnLbnDblclkListBookmarks() {
  ...
  if (!is_outside) {
    CFamiTrackerView *pView = static_cast<CFamiTrackerView *>(
        static_cast<CMainFrame *>(AfxGetMainWnd())->GetActiveView());
...
void CBookmarkDlg::OnBnClickedCheckBookmarkPersist() {
  m_bPersist = static_cast<CButton *>(GetDlgItem(IDC_CHECK_BOOKMARK_PERSIST))
                   ->GetCheck() == BST_CHECKED;
}
