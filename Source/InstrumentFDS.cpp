...
const char TEST_WAVE[] = {00, 01, 12, 22, 32, 36, 39, 39, 42, 47, 47, 50, 48,
                          51, 54, 58, 54, 55, 49, 50, 52, 61, 63, 63, 59, 56,
                          53, 51, 48, 47, 41, 35, 35, 35, 41, 47, 48, 51, 53,
                          56, 59, 63, 63, 61, 52, 50, 49, 55, 54, 58, 54, 51,
                          48, 50, 47, 47, 42, 39, 39, 36, 32, 22, 12, 01};

const int FIXED_FDS_INST_SIZE = 2 + 16 + 4 + 1; // // //

LPCTSTR CInstrumentFDS::SEQUENCE_NAME[] = {
    _T("Volume"), _T("Arpeggio"), _T("Pitch"), _T("Hi-pitch"), _T("(N/A)")};

CInstrumentFDS::CInstrumentFDS()
    : CSeqInstrument(INST_FDS), // // //
      m_iModulationSpeed(0), m_iModulationDepth(0), m_iModulationDelay(0),
      m_bModulationEnable(true), m_iModulation() {
...
void CInstrumentFDS::StoreInstSequence(CInstrumentFile *pFile,
                                       const CSequence *pSeq) {
...
CSequence *CInstrumentFDS::LoadInstSequence(CInstrumentFile *pFile) const {
  int SeqCount = CModuleException::AssertRangeFmt(pFile->ReadInt(), 0U, 0xFFU,
                                                  "Sequence item count", "%u");
  int Loop = CModuleException::AssertRangeFmt(
      static_cast<int>(pFile->ReadInt()), -1, SeqCount - 1,
      "Sequence loop point", "%u");
  int Release = CModuleException::AssertRangeFmt(
      static_cast<int>(pFile->ReadInt()), -1, SeqCount - 1,
      "Sequence release point", "%u");

  CSequence *pSeq = new CSequence();
  pSeq->SetItemCount(SeqCount > MAX_SEQUENCE_ITEMS ? MAX_SEQUENCE_ITEMS
                                                   : SeqCount);
...
void CInstrumentFDS::StoreSequence(CDocumentFile *pDocFile,
                                   const CSequence *pSeq) {
  // Store number of items in this sequence
...
CSequence *CInstrumentFDS::LoadSequence(CDocumentFile *pDocFile) const {
  ...
  unsigned int LoopPoint = CModuleException::AssertRangeFmt(
      pDocFile->GetBlockInt(), -1, SeqCount - 1, "Sequence loop point", "%i");
  ;
  unsigned int ReleasePoint = CModuleException::AssertRangeFmt(
      pDocFile->GetBlockInt(), -1, SeqCount - 1, "Sequence release point",
      "%i");
  ;

  // CModuleException::AssertRangeFmt(SeqCount, 0, MAX_SEQUENCE_ITEMS, "Sequence
  // item count", "%i");
...
int CInstrumentFDS::GetModulationSpeed() const { return m_iModulationSpeed; }
...
int CInstrumentFDS::GetModulationDepth() const { return m_iModulationDepth; }
...
int CInstrumentFDS::GetModulationDelay() const { return m_iModulationDelay; }
...
bool CInstrumentFDS::GetModulationEnable() const { return m_bModulationEnable; }
...
void CInstrumentFDS::SetSeqIndex(int Index, int Value) { ASSERT(false); }
