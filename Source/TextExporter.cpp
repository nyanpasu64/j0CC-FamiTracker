                                           ...
                                           bool bNoise) // // //
{
  ...
  for (unsigned int e = 0; e < nEffects; ++e) {
    if (stCell.EffNumber[e] == 0) {
      ...
      tmp.Format(_T(" %c%02X"), EFF_CHAR[stCell.EffNumber[e]],
                 stCell.EffParam[e]);
      s += tmp;
    }
  }
  ...
  {                                                                            \
    ...
    if (symbol_ != _T(x)) {                                                    \
      sResult.Format(_T("Line %d column %d: expected '%s', '%s' found."),      \
                     t.line, t.GetColumn(), _T(x), symbol_);                   \
      return sResult;                                                          \
    }                                                                          \
  }
...
const CString &CTextExport::ExportFile(LPCTSTR FileName,
  ...
  s.Format(_T("# Song information\n"
              ...
              "%-15s %s\n"
              "\n"),
           CT[CT_TITLE], ExportString(pDoc->GetSongName()), CT[CT_AUTHOR],
           ExportString(pDoc->GetSongArtist()), CT[CT_COPYRIGHT],
           ExportString(pDoc->GetSongCopyright()));
  f.WriteString(s);
